/-
  ...
  (`chartCoverOverlapData`), build a `Coboundaries` — the sup-norm Čech complex on
  bounded-holomorphic cochains — whose `supH1` is `ℂ`-linearly iso to the germ-class `cechH1`. The
  `Coboundaries` fields split into three pieces:
    ...
    we add the 0- and 2-cochain spaces and the Čech differentials. (b) the `leray` field —
    disk-acyclicity (`DbarDiskCohomology.dbar_solvable_ball`) assembled per-overlap + partition of
    unity (separate file). (c) the comparison `cechH1 ≃ₗ supH1` — the germ↔`BddHol` cochain iso
    (separate file).

  The cross-chart subtlety of (a): a 0-cochain assigns each cover set `a` a bounded-holomorphic
  function on its OWN chart-image `(chartAt (coverCenter a)) '' (chartOpen a)` (chart-`a`
  coordinates). The Čech `δ⁰` on the overlap `(a,b)` is `(δ⁰f)_{ab} = f_b − f_a` read in a COMMON
  chart — here chart `a` (since `Uov (a,b)` is read in chart `a`). So `f_a` restricts directly, but
  `f_b` must be transported from chart-`b` to chart-`a` coordinates through the holomorphic
  transition `φ_a ∘ φ_b⁻¹` (which is `AnalyticOn` on the overlap, the `ω`-manifold's chart change).
  Building this transition transport on `BddHol`, with boundedness + continuity of the resulting
  `δ⁰`, is the genuine content of (a) and the next work. The `CechH0`/`HoloRep` chart-change atoms
...
The Čech `δ¹` lands in the 2-cochains over triple overlaps `(a,b,c)`, read (like the pairwise data)
in the FIRST chart `a`. Cover side `C²cov` is bounded-holomorphic on the OPEN triple chart-image;
the shrinking side `C²` is bounded-continuous on the COMPACT inner triple chart-image (where the
Montel restriction lands), matching `DiskOverlapData.Cshr`'s `→ᵇ` style. -/
