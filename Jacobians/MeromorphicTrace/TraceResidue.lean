...
already built in `Jacobians.ResidueCalculus.Residue` (`resAt`).
...
machinery (`Jacobians.MeromorphicTrace.TraceForm` / `Jacobians.MeromorphicTrace.TracePullback`, the S8 layer) pushes forward only
...
namespace Jacobians.MeromorphicTrace.TraceResidue
...
end Jacobians.MeromorphicTrace.TraceResidue
