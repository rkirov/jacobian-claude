...
`Jacobians.MeromorphicTrace.TraceResidue` — and the cover `F = MeromorphicFunction.toRiemannSphere` (in
`Jacobians.ProperDegree.ToSphereGeneral`), they assemble into
...
namespace Jacobians.MeromorphicTrace.MeromorphicTrace

open Jacobians.Dolbeault Jacobians.MeromorphicTrace.TraceResidue
...
end Jacobians.MeromorphicTrace.MeromorphicTrace
