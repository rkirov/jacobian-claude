...
Target (`Jacobians/ProperDegree/DegreeOneSphere.lean`):
```
...
structure). The bridge is the classical contrapositive route from `Jacobians.SphereTopology.GenusSphereBackward`:
...
* **`S²` is simply connected** — now *unconditional* in the repo
  (`Jacobians.SphereTopology.VanKampen` proves `twoOpenVanKampen_holds`, giving the global instance
...
(`Jacobians.SphereTopology.VanKampen`, `twoOpenVanKampen_holds`). Composing with
