...
`MeromorphicFunction.HasSingleSimplePole` is defined in `Jacobians.Meromorphic.MeromorphicLiouville`
...
arbitrary junk value (see `Jacobians.Meromorphic.MeromorphicLiouville`) — which would make
...
**topological**; the bridge is the contrapositive route (`Jacobians.SphereTopology.GenusSphereBackward`):
