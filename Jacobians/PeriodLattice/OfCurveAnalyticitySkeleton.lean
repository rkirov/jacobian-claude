...
-- UPSTREAM to `Jacobians/Path/SmoothPathCore.lean` (same namespace
-- `Jacobians.OfCurveSkeleton`); they remain available here transitively
-- via `import Jacobians.JacobianConstruction.PeriodLattice` → `Jacobians.Path.SmoothPathCore`.
...
-- `Jacobians/Path/SmoothPathCore.lean` (same namespace); available here
-- transitively.
...
-- `localLift` has moved UPSTREAM to `Jacobians/Path/SmoothPathCore.lean`
...
chart-transition smoothness — built up in `Jacobians/Path/SmoothPath.lean`).
...
(c) `periodVec_smoothPath_eq_periodVec_concat_mod_lattice`: the
    ...
    proven infrastructure in `Jacobians/Path/LineIntegral.lean` and
    `PeriodLattice.lean`). -/
...
-- have moved UPSTREAM to `Jacobians/Path/SmoothPathCore.lean` (same namespace);
-- available here transitively via `import Jacobians.JacobianConstruction.PeriodLattice`.
...
* `IsClosedSmoothLoop (concat γ₁ (reverse γ₂))`: needs ChartBallPath
  ...
  (proven infrastructure in `Jacobians/Path/LineIntegral.lean` and
  `Jacobians/JacobianConstruction/PeriodLattice.lean`).
