...
`SerreResidueRamifiedRealSlitAssembly.lean` reduced the residue theorem `∑Res = 0`, for the real
cover, to the single obligation `RealCoverSlitSectionGeometry`: at each finite pole-value centre
`c`, a slit `Sset` accumulating at `c` off the branch locus, the per-preimage §5 cluster data `Cl`,
and per slit value `z ∈ Sset` the §5 normal-form section facts `RealSlitSectionData`
(`hcs_sec`/`hcs_np`/`hwithin`/ `hcross`/`hsrc`).
...
The §5 atom's inverse property `η (s a) = a` and the normal form `F (chart.symm w) = c + η wᵐ` hold
only **near `a = 0` / `w = chart p`** (a local biholomorphism domain). For a *fixed* slit value `z`
and `w` near `z`, the cluster argument is `a(w) = ζʲ · w₀ w`, which tends to `ζʲ · w₀ z` — NOT to
`0` unless `z` is close to `c` (`w₀ z = (z − c)^{1/m} → 0` as `z → c`). The resolution is the
**shrunk slit**: choose `Sset ⊆ {z | z − c ∈ slitPlane}` small enough (per preimage) that every
cluster argument `ζʲ · w₀ z` stays in the §5 inverse-property domain. Then, for `z` interior to the
shrunk slit, a *full* ℂ-neighbourhood of `z` keeps the cluster arguments in that domain, so the
eventual section facts (`hcs_sec`, `hsrc`) hold.
...
1. **The cluster data `Cl`** — a `ClusterTraceData` per preimage via `ClusterTraceData.ofNormalForm`
on the shrunk slit, whose ONLY non-mechanical input is the **`Rem` symmetric-function descent**: for
the analytic remainder `ppR` of the straightened integrand, the trace
`Rem z = ∑_{j<m} ppR(ζʲ·w₀ z)·deriv(ζz ↦ ζʲ·w₀ ζz) z` is **analytic at `c`** (`hRem_an`). This is
the standard "trace of a holomorphic form is holomorphic"; closed form
`Rem z = ∑_{k≥0} a_{m(k+1)-1}·(z − c)^k` (`a_n` the Taylor coefficients of `ppR` at `0`). It is NOT
in Mathlib and has been carried as a *data hypothesis* throughout the ramified-residue subtree
(`RamifiedSheetData`/`ClusterTraceData`); it is the single genuinely-new analytic lemma. Its
cleanest formulation is the reusable atom: *a `ζ`-invariant analytic germ `P` (`P(ζ·u) = P(u)`, `ζ`
a primitive `m`-th root) factors as `P = Q ∘ (·^m)` with `Q` analytic* — equivalently the symmetric
trace `z ↦ ∑_{j<m} G(ζʲ·(z − c)^{1/m})` is analytic at `c` for `G` analytic at `0`.

2. **The shrunk slit `Sset` + the germ hypotheses** — a slit `Sset ⊆ {z | z − c ∈ slitPlane}` shrunk
near `c` (off the finitely-many branch values, still accumulating at `c`) on which the
`SlitSectionGerm` smallness hypotheses hold for every `z₀ ∈ Sset` (the cluster arguments `ζʲ·w₀ z₀`
lie in the §5 inverse-property domain). This is mechanical (continuity +
`cpow_slitBranch_tendsto_zero` + the §5 atom's open domains + T2 separation of the preimages) but
voluminous.

## ⚠ Soundness

The §5 sections are genuine (`exists_clusterSplit_at_fibrePoint`). The shrunk slit is a genuine slit
(still accumulating at `c`, off the branch locus, simply connected). The cluster sheets are the
genuine `s(ζʲ·w₀ z)` (NOT the first-order `wp + ζʲ·w₀ z`). Slit values are regular (off-branch). `D`
is the whole fibre (#17). No custom axiom, no unproved obligation on a false statement, no
...
slit value and tends to `0` as `z → c` (`(z − c) → 0`, `0^{1/m} = 0`). This is the mechanism that
lets the cluster arguments `ζʲ · w₀ z` be pushed into the §5 inverse-property domain by shrinking
the slit. -/

/-- **The cpow slit branch is continuous and `→ 0` at the centre.** For `m ≥ 1`, the principal
branch `w₀ z := (z − c)^{(m:ℂ)⁻¹}` tends to `0` as `z → c`. (`cpow` is continuous at `0` for a
positive real exponent: `0^{1/m} = 0` since `1/m ≠ 0`.) -/
...
A non-pole `x` (`0 ≤ orderAtPoint x`) of a meromorphic function has a whole neighbourhood of
non-poles: `toRiemannSphere` is continuous at `x` (it equals `coe ∘ holoRepr` near a non-pole) with
finite value `coe (holoRepr x) ≠ ∞`, so points near `x` map into `{∞}ᶜ`, hence are non-poles
(`toRiemannSphere_of_pole` contrapositive). This is the mechanism that makes `hcs_np` reachable: for
the shrunk slit, the cluster section point lies in the non-pole neighbourhood of its fibre preimage
`p`. -/

/-- **Non-pole is an open condition.** If `x` is a non-pole (`0 ≤ f.orderAtPoint x`), then every
point in a neighbourhood of `x` is a non-pole. -/
...
The §5 atom `exists_clusterSplit_at_fibrePoint` provides `η`/`s` with the inverse property
`η (s a) = a` near `a = 0` and the normal form `f.holoRepr (chart.symm w) = c + η wᵐ` near
`w = chart p`. For a slit value `z₀` whose cluster arguments `ζʲ · w₀ z₀` lie near `0`, the section
property `f.holoRepr (chart.symm (clusterSheet s ζ w₀ j w)) = w` holds for `w` in a *full
ℂ-neighbourhood* of `z₀` (the cluster arguments stay near `0`, the cluster points stay near
`chart p`, and the slit power identity holds), by pulling back the §5 atom's eventual facts along
the continuous `w ↦ ζʲ · w₀ w`. -/

/-- **The eventual §5 section property near a slit value `z₀`.**  Given the §5 normal-form data
(`η`/`s`, the inverse property `hinv` valid near `a₀ := ζʲ · w₀ z₀`, the normal form `hnf` valid
near `s a₀`, and the slit power identity `hpow` valid near `z₀`), the cluster sheet point is a
genuine `holoRepr`-section of `f` near `z₀`:

> `∀ᶠ w in 𝓝 z₀, f.holoRepr (chart_p.symm (clusterSheet s ζ w₀ j w)) = w`.

This is the §5 section clause of `exists_clusterSplit_at_fibrePoint` (`clusterSheet_sect`),
localised at `z₀` by the continuity pullback of the atom's `𝓝 0` / `𝓝 (chart p)` eventual facts. -/
theorem eventually_holoRepr_clusterSheet_eq {X : Type*} [TopologicalSpace X] [ChartedSpace ℂ X] 
    (f : MeromorphicFunction X) {c : ℂ} {p : X} {m : ℕ}
...
Within one preimage's cluster, the `m` sheet points are distinct because the §5 straightening
coordinate `η` recovers the sheet argument: `η (s (ζʲ · w₀ z)) = ζʲ · w₀ z`. The proven
...
At *distinct* preimages `p ≠ p'`, the clusters are disjoint: the cluster sheet points cluster at
`p`, `p'` respectively, which are T2-separated. We package the separation as a standalone fact: two
points lying in disjoint sets are distinct. At the assembly the two cluster section points lie in
disjoint neighbourhoods of `p` and `p'` (the cluster points are near their respective preimages on
the shrunk slit). -/

/-- **Cross-cluster separation from disjoint neighbourhoods.** If `q ∈ U`, `q' ∈ U'`, and `U`, `U'`
are disjoint, then `q ≠ q'`. Used with `U`/`U'` the T2-separating neighbourhoods of distinct
preimages `p ≠ p'` (the cluster section points lie near their preimages on the shrunk slit). -/
theorem ne_of_mem_disjoint {X : Type*} {q q' : X} {U U' : Set X} (hq : q ∈ U) (hq' : q' ∈ U')
...
We package, for one slit value `z₀` and one fibre preimage `i`, exactly the §5 normal-form data the
five section facts consume — the straightening coordinate `η` matching the cluster data's local
inverse `(Cl i).s`, the §5 eventual facts (inverse near the cluster argument, normal form near the
cluster point, slit power near `z₀`), the chart-target locality of the cluster sheet value, the §5
inverse recovery, and the smallness witnesses making the cluster section point a non-pole near its
preimage and the clusters cross-separated. From a slit-wide such bundle, the builder assembles
`RealSlitSectionData`, discharging all five fields. This isolates the residue obligation's section
half to **this germ data per `(z₀,i)`** — the genuine Forster §5 normal-form geometry, with the
cluster data `Cl` (carrying the `Rem` symmetric-function descent) supplied separately. -/

/-- **The per-`(z₀,i)` §5 section germ data** (the precise per-preimage section content at one slit
value `z₀`). For the whole-fibre `D = realFibreData g hdiv c hnp`, the cluster data `Cl`, a slit
value `z₀`, and a preimage `i`, bundles:

* `η` — the §5 straightening coordinate, with the inverse property `hinv` valid near each cluster
  argument `(Cl i).ζ ^ j · (Cl i).w₀ z₀` (`j < mult i`), the normal form `hnf` valid near each
  cluster point `(Cl i).s ((Cl i).ζ ^ j · (Cl i).w₀ z₀)`, and the slit power identity `hpow` near
  `z₀`;
* `hs_cont`/`hw₀_cont` — continuity of the §5 local inverse `(Cl i).s` at the cluster arguments and
  of the branch `(Cl i).w₀` at `z₀`;
* `htgt` — the cluster sheet value `(Cl i).s (cluster arg)` lies in the (open) chart target at
  `D.xs i`;
* `hnonpole` — the cluster section point is a non-pole at `z₀` (it lies near the non-pole preimage
  `D.xs i` on the shrunk slit);
* `hsep` — cross-cluster separation: at distinct preimages `i ≠ i'` the cluster section points at
  `z₀` are distinct (the clusters cluster at distinct T2-separated preimages on the shrunk slit). -/
...
/-- **`RealSlitSectionData` from a per-preimage `SlitSectionGerm` family at `z₀`.** Given, at the
slit value `z₀`, a `SlitSectionGerm` for every fibre preimage `i` (the genuine Forster §5
normal-form section geometry), and the multiplicity match
`hmult : ∀ i, (Cl i).m = (realFibreData …).mult i`, build the `RealSlitSectionData`: each of the
five section fields is discharged from the germ data via the section-fact toolkit
(`eventually_holoRepr_clusterSheet_eq`, `eventually_clusterSheet_mem_target`,
`clusterSection_within_cluster_inj`, `clusterArg_inverse_self`). This makes the **section half** of
the residue obligation `RealCoverSlitSectionGeometry` done; the only separately-supplied content is
the cluster data `Cl` (carrying the `Rem` symmetric-function descent). -/
noncomputable def RealSlitSectionData.ofSlitSectionGerm {ω₀ : HolomorphicOneForms X}
  ...
  hcs_sec := fun i j => by
    -- the §5 section property near z₀, via the germ's eventual inverse/normal-form/slit-power
    -- facts.
