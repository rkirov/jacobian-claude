...
/-!
# Building `CoherentTraceSelection` from the moving-fibre self-coherence (Miranda §VIII.3)

`Jacobians.Dolbeault.FormTraceMovingFibre` proved the monodromy heart: a `MovingCoherenceDatum` at a
base value `b₀` yields the **local self-coherence**
`valueChartTrace ω₀ f Φ =ᶠ[𝓝[≠] b₀] (fibreTrace ω₀ f D).traceCoeff` of the geometric trace with a
*fixed* fibre — and that datum is now constructible from the irreducible monodromy core (the
continuously-varying index bijection, `MovingCoherenceDatum.ofBijection`).
...
* the **finite glue** `hglue_fin i` is exactly the local self-coherence at the pole-value `cs i`
  with the fixed fibre the **pole sub-fibre** `fibreReg hac (cs i)` (honest soundness: NOT the full
  fibre — the pole/regular separation is folded into the selection `Φ`, encoded by the index
  bijection at `cs i`);
* the **off-exceptional coherence** `hreg z` is the local self-coherence at a regular value `z` with
  a regular fibre `Dreg z` (with `g`'s chart-pullback analytic at each fibre point);
...
The finite glue `hglue_fin i` and the off-exceptional coherence `hreg z` of `CoherentTraceSelection`
are *exactly* the punctured/full self-coherence
`MovingCoherenceDatum.coherent_punctured`/`.coherent` with the appropriate fixed fibre. We record
the two specializations: the finite glue against the **pole sub-fibre** `fibreReg hac (cs i)`, and
the regular glue against a regular fibre `Dreg`. -/

/-- **Finite glue from a moving datum at a pole-value.** A `MovingCoherenceDatum` at `cs i` whose
fixed fibre is the pole sub-fibre `fibreReg hac (cs i)` gives the finite glue
...
`cs i`). -/
theorem glue_fin_of_movingDatum {Φ : (b : ℂ) → FibreRegularData g f b}
...
/-- **Off-exceptional coherence from a moving datum at a regular value.** A `MovingCoherenceDatum`
at a regular value `z` whose fixed fibre `C.D` has `g`'s chart-pullback analytic at each fibre point
gives the `hreg`-witness: the regular fibre `C.D` with the full-neighbourhood coherence
...
sub-fibre), the **per-regular-value moving data** (regular fibre + `g`-analyticity), the `∞`-glue,
junk-freeness, and the genus-`0` continuation. The finite glue and off-exceptional coherence are
then *derived* from the moving data (`glue_fin_of_movingDatum`/`hreg_of_movingDatum`); the rest is
carried verbatim. -/
...
`hreg_of_movingDatum` — the proved §VIII.3 self-coherence); the remaining fields carry over
verbatim. -/
...
The `MovingCoherenceFamily` obligation is *satisfiable*, not a disguised `False`: for the **empty
pole set** (`α = ω₀·g` globally holomorphic) the empty fibre selection gives a family — the
per-regular-value data is the empty datum (`movingCoherenceDatum_empty`, `valueChartTrace ≡ 0`), no
finite pole-values, the empty `∞`-trace (`recipCoeff 0 ≡ 0`), vacuous junk-freeness, and the
vanishing genus-`0` continuation. This confirms the family produces a genuine `∑Res = 0`, mirroring
`coherentTraceSelection_empty`. -/

/-- **The empty moving coherence family.** For the empty pole set, the empty fibre selection
assembles into a `MovingCoherenceFamily`: per-regular-value empty data, vacuous finite/∞-pole
fields, and the vanishing genus-`0` continuation. The honest non-vacuity witness. -/
