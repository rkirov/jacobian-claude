...
* `valueChartTrace_eq_sphereSheetFibreTrace` (`FormTraceBundleBridge.lean`) — at a regular slit
  value `z`, the geometric trace equals the `traceCoeff` of the **whole** sphere fibre `F⁻¹(coe z)`
  (`LocalSheetSystem S` of `deg f` moving sheets);
* `fibreTrace_eventuallyEq_movingSum` (`FormTraceMovingFibre.lean`) — that `traceCoeff` reads, AT
  `z`, as the moving fibre sum `∑ₖ chartIntegrand ω₀ g (S.sheet k (coe z)) … · deriv …` along the
  sheets `holoReprSheet`;
...
* **`valueChartTrace_eq_clusterSum_of_reindex`** — the **pure-reindexing reduction** of
  `hgeom_fibre`: given (i) the regular-value sphere coherence at `z`, (ii) a bijection
  `e : (Σ i, Fin (D.mult i)) ≃ Fin S.n` of the cluster `Σ`-index with the sphere sheets, and (iii)
  the per-`(i,j)` summand equality matching the cluster summand to the `e ⟨i,j⟩`-th moving summand,
  the full-fibre cluster identity holds. This is the combinatorial spine: it turns the whole-fibre
  moving sum into the per-preimage cluster double sum.
...
* **`clusterSummand_eq_sphereSummand`** — the per-`(i,j)` chart reconciliation: the fixed-preimage
  cluster summand equals the moving sphere summand at the coincident point (via
  `movingSummand_chartIndep` + the section-derivative agreement).

* **`valueChartTrace_eq_clusterSum_of_sphereReindex`** — the moving-sheet form of the reduction
  (working directly with a sphere sheet system `S` at `coe z`, the `holoReprSheet` moving sum).

* **`ClusterReindexData`** + **`valueChartTrace_eq_clusterSum_of_clusterReindexData`** — the
...
* **`FibreClusterReindex.ofClusterReindexFamily`** — builds the whole per-centre
  ...
  the routine fields + a slit-wide family `∀ z ∈ Sset, ClusterReindexData …`.

* **`ClusterReindexData.sum_mult_eq_sheetCount`** / **`hderiv_match_of_section`** — the soundness
...
per-preimage clusters — the genuine §VIII.3/Forster §4–5 finite-cover clustering topology, with
every algebraic/chart-reconciliation step proven.
...
chart-reconciliation (`clusterSummand_eq_sphereSummand`), not assumed. Two soundness facts confirm
the structure is genuine and not single-preimage:

* `ClusterReindexData.sum_mult_eq_sheetCount` — the bijection field **forces** the
  conservation-of-number identity `∑ᵢ D.mult i = S.n` (`= deg f`); the structure is therefore
  meaningfully multi-preimage, not a disguised single-preimage placeholder, and its `e`/`hpoint`
  fields carry genuine content.
* `hderiv_match_of_section` — the `hderiv_match` field is **derivable** (holomorphic-local-inverse
  uniqueness), so the only irreducible fields are the bijection `e` and the point coincidence
  `hpoint` (the genuine §17.9/Forster §4 conservation-of-number / properness topology).
...
`valueChartTrace_eq_sphereSheetFibreTrace` writes the geometric trace at a regular slit value `z` as
a sum over the whole sphere fibre `F⁻¹(coe z)` —
`(fibreTrace ω₀ f (ofSphereSheetSystem S …)).traceCoeff z`, a single sum over the `deg f` moving
sheets `Fin S.n`. The full-fibre cluster identity rewrites this as the **double** sum over preimages
× cluster index `Σ i, Fin (D.mult i)`. The two index sets have the same cardinality (conservation of
number: `∑ᵢ mᵢ = deg f`), and the bijection `e` matches each cluster sheet to a moving sheet. Given
the matching, the reduction is a pure reindexing. -/

/-- **Pure-reindexing reduction of the full-fibre cluster identity.** Let
`D : FibreRamifiedData g f c` be the pole fibre, `Cl i : ClusterTraceData ω₀ g (D.xs i) c S` the
per-preimage cluster data, and at a slit value `z` let `T : FibreTrace` be any sphere-fibre trace
with `valueChartTrace ω₀ f Φ z = T.traceCoeff z` (the regular-value sphere coherence). Given a
bijection `e : (Σ i, Fin (D.mult i)) ≃ T.ι` of the cluster `Σ`-index with the sheets, and the
per-`(i,j)` summand equality matching the cluster summand to the `e ⟨i,j⟩`-th sheet summand of
`T.traceCoeff`, the full-fibre cluster identity holds at `z`:

> `valueChartTrace ω₀ f Φ z = ∑ᵢ ∑_{j<mᵢ} chartIntegrand ω₀ g (D.xs i) (clusterSheet (Cl i).s … j z)
>     · deriv (clusterSheet (Cl i).s … j z)`.

*Proof.* `T.traceCoeff z = ∑ₖ T.coeff k (T.sheet k z)·deriv (T.sheet k) z`; reindex the `∑ₖ` along
`e⁻¹` into a `Σ`-sum, then the summand equality identifies each `e ⟨i,j⟩`-summand with the cluster
summand; flatten `Σ i, Fin (D.mult i)` into the `∑ᵢ ∑_{j ∈ range (mult i)}` double sum. -/
...
**fixed preimage** `D.xs i`'s chart) with the moving sphere summand (read in the **moving sheet
point**'s own chart, along the section `holoReprSheet k`). Both are pushforwards of `α = ω₀·g` along
a local section of `f` through the *same* fibre point `q = S.sheet k (coe z)`; they agree by the
source-chart independence of the trace summand (`movingSummand_chartIndep`) once the two sections
germ-agree (both are holomorphic right-inverses of `f`'s chart-pullback through `q`). This is the
§VIII.3 fact that "the trace `Tr_F α` is a well-defined form", applied at the cluster.

We isolate the reconciliation as a standalone lemma taking the genuine geometric residuals as
hypotheses: the point coincidence (`cs z = q`), the section-derivative agreement
(`deriv (chart_q ∘ cs) z = deriv (chart_q ∘ holoReprSheet k) z`, the holomorphic-local-inverse
uniqueness), and the differentiability data feeding `movingSummand_chartIndep`. -/

/-- **Per-summand chart reconciliation at a cluster point.** Let `q := S.sheet k (coe z)` be a
sphere sheet point at the regular slit value `z`, and `cs : ℂ → X` a local section of `f` through
`q` with `cs z = q` and chart coordinate `chart_{a} (cs w) = cw w` near `z` (so `cw z = chart_a q`),
where `a` is the fixed preimage chart base. Then the **fixed-preimage cluster summand** equals the
**moving sphere summand**:

> `chartIntegrand ω₀ g a (cw z)·deriv cw z
> = chartIntegrand ω₀ g q (chart_q (holoReprSheet k z))·deriv (fun w => chart_q (holoReprSheet k w))
  z`,

given: `q ∈ chart_a.source`; `cs` continuous at `z`; the chart-pullbacks `chart_q ∘ cs`,
`chart_a ∘ cs` differentiable at `z`; the two chart transitions (`a ↔ q`) differentiable both ways;
and the section-derivative agreement `deriv (chart_q ∘ cs) z = deriv (chart_q ∘ holoReprSheet k) z`
(the holomorphic-local-inverse uniqueness, both sections being right-inverses of `f` through `q`).
Pure chart algebra: rewrite the cluster summand as the `a`-chart `movingSummand` of `cs`, apply
...
theorem clusterSummand_eq_sphereSummand (ω₀ : HolomorphicOneForms X) (g : X → ℂ)
  ...
  -- arguments and the chart match (rewrite `q` via `hq`); the section derivs match
  -- (`hderiv_match`).
...
directly from a sphere sheet system `S` at `coe z`, a bijection
`e : (Σ i, Fin (D.mult i)) ≃ Fin S.n`, and the per-`(i,j)` summand match **in the moving
(holoReprSheet) form** — exactly what `clusterSummand_eq_sphereSummand` produces. -/

/-- **The full-fibre cluster identity from a sphere sheet system + bijection (moving-sheet form).**
At a regular slit value `z` with a sphere sheet system `S` of `F = f.toRiemannSphere` at `coe z`
(regular fibre — `hderiv`, `hmero`), the regular-value coherence
`valueChartTrace ω₀ f Φ z = (fibreTrace ω₀ f (ofSphereSheetSystem S …)).traceCoeff z`, a bijection
`e : (Σ i, Fin (D.mult i)) ≃ Fin S.n`, and the per-`(i,j)` summand equality matching the cluster
summand to the `e ⟨i,j⟩`-th **moving (holoReprSheet) summand**, the full-fibre cluster identity
holds at `z`.

*Proof.* Rewrite the coherence's RHS via `fibreTrace_eventuallyEq_movingSum` (at `z`, sections
`holoReprSheet`) into the moving fibre sum `∑ₖ chartIntegrand ω₀ g (S.sheet k (coe z)) …·deriv …`,
then apply `valueChartTrace_eq_clusterSum_of_reindex` with `T` the moving sum (packaged as the
trivial `FibreTrace` whose `traceCoeff` *is* that sum) — i.e. directly reindex the moving sum. -/
...
regular slit value `z` as a named datum, with the chart-reconciliation half DISCHARGED. Its fields
are exactly the conservation-of-number content: a sphere sheet system `S` at `coe z` (with the
regular-value `hderiv`/`hmero` and the coherence `hcoh`), the **bijection**
`e : (Σ i, Fin (D.mult i)) ≃ Fin S.n` between the cluster `Σ`-index and the `deg f` sphere sheets,
the **point coincidence** that each cluster sheet point is the matched moving sheet point, and the
routine analytic data feeding the chart-reconciliation (the cluster section's differentiability +
the section-derivative agreement, the holomorphic-local-inverse uniqueness). From it we prove
`hgeom_fibre` at `z`.

The cluster section at preimage `i`, cluster index `j` is
`clusterSection D i j Cl := fun w ↦ chart_{D.xs i}.symm (clusterSheet (Cl i).s … j w)` — the genuine
`clusterSheet` point, lifted to `X` through the chart at the fixed preimage `D.xs i`. -/
...
noncomputable def clusterSection {ω₀ : HolomorphicOneForms X} {g : X → ℂ}
...
conservation-of-number content, chart-reconciliation discharged). At `z`, a sphere sheet system `S`
of `F = f.toRiemannSphere` at `coe z` with the regular-value coherence, a bijection
`e : (Σ i, Fin (D.mult i)) ≃ Fin S.n`, and, for each preimage `i` and cluster index `j` (writing
...
* the differentiability data `hmem_a`/`hcs_cont`/`hcsP_diff`/`htrans_diff`/`htrans_diff_inv`
  (routine analyticity of `clusterSheet` + the chart transitions);
* `hderiv_match` — the **section-derivative agreement** `deriv (chart_q ∘ cs) z = deriv (chart_q ∘
  holoReprSheet (e ⟨i,j⟩))
  z` (the holomorphic-local-inverse uniqueness: both `cs` and `holoReprSheet (e
  ⟨i,j⟩)` are right-inverses of `f` through `q`).

This is the single residual the `hoff_cs`-free residue-theorem route still needs at each slit value;
everything else (the residue calculus, the per-cluster collapse, the meromorphy reduction, the
off-centre/∞ machinery, the chart reconciliation) is proven. -/
structure ClusterReindexData {ω₀ : HolomorphicOneForms X} {g : X → ℂ} {f : MeromorphicFunction X}
  ...
  /-- The chart transition `chart_{D.xs i} ∘ chart_{cs z}.symm` is differentiable at the cluster
  point. -/
  ...
  /-- **Section-derivative agreement** (holomorphic-local-inverse uniqueness): the cluster section
  and the matched moving sheet `holoReprSheet (e ⟨i,j⟩)` have the same self-chart-pullback
  derivative at `z` (both are right-inverses of `f` through the coincident point). -/
...
/-- **`hgeom_fibre` at `z` from a `ClusterReindexData`.** The full-fibre cluster identity at the
regular slit value `z` follows from the clustering datum: the per-`(i,j)` summand match is
...
slit `Sset` (with matching multiplicity + the residue split), the eventual off-centre analyticity,
the slit accumulation, the finite pole-order bound, **and** a `ClusterReindexData` at every slit
value `z` (the conservation-of-number bijection + point coincidence), the per-centre fibre-cluster
reindexing `FibreClusterReindex` holds. The hard `hgeom_fibre` field is discharged pointwise on the
slit by `valueChartTrace_eq_clusterSum_of_clusterReindexData`; all other fields are passed through.
-/
noncomputable def FibreClusterReindex.ofClusterReindexFamily {ω₀ : HolomorphicOneForms X}
    {g : X → ℂ}
...
(`= S.n`, the number of sphere sheets). A `ClusterReindexData` at a genuinely ramified fibre
(several preimages with multiplicities `> 1`) is therefore exactly a witness that the `deg f` moving
sheets partition into the per-preimage clusters of total size `∑ᵢ mᵢ` — the genuine §17.9/Forster §4
...
the genuine conservation-of-number content — no single-preimage restriction, not a vacuous
placeholder. -/
theorem ClusterReindexData.sum_mult_eq_sheetCount {ω₀ : HolomorphicOneForms X} {g : X → ℂ}
  ...
  rwa [Fintype.card_sigma, Fintype.card_fin,
    Finset.sum_congr rfl (fun i _ => Fintype.card_fin (D.mult i))]
    at hcard

/-! ## The section-derivative agreement is DERIVABLE (holomorphic-local-inverse uniqueness)

The `hderiv_match` field of `ClusterReindexData` is **not** an extra assumption: it is the genuine
fact that two holomorphic local sections of `f` through the *same* fibre point germ-agree
(uniqueness of the holomorphic local inverse). We isolate the discharger, so `hderiv_match` reduces
to the two sections being genuine local sections of `f.holoRepr` through the coincident point `q`
(the `clusterSection` section property is `clusterSheet_sect`; the `holoReprSheet` one is
`holoReprSheet_section`). This confirms the only irreducible fields of `ClusterReindexData` are the
**bijection** and the **point coincidence** (the conservation-of-number content). -/

/-- **The section-derivative agreement from the two right-inverse properties**
(holomorphic-local-inverse uniqueness). Let `q := s₁ z` be a regular non-pole with
`f.holoRepr q = z` (so the chart-pullback `φ = f.holoRepr ∘ chart_q.symm` is analytic at `chart_q q`
with nonzero derivative). If two sections `s₁ s₂ : ℂ → X` both pass through `q` at `z`
(`s₁ z = q = s₂ z`), are continuous at `z`, lie in `q`'s chart source near `z`, and are sections of
`f.holoRepr` near `z` (`f.holoRepr (sₖ w) = w`), then their self-chart pullbacks have equal
derivatives at `z`:
...
exhibited). For a genuine meromorphic numerator `g`, an `AdaptedFRamified` datum `A`, and at each
finite pole-value centre `A.cs i` a per-centre `FibreClusterReindex` (e.g. built via
`FibreClusterReindex.ofClusterReindexFamily` from a slit-wide family of `ClusterReindexData`), the
