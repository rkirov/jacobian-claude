...
The Forster §5 cluster split (`exists_clusterSplit`, `SerreResidueRamifiedClusterSplit.lean`)
consumes, at a ramification preimage `p` of a finite value-centre `c`, the **analytic-order
hypothesis** `analyticOrderAt (F − c) (chart_p p) = m`, where `F = f.holoRepr ∘ chart_p.symm` is the
*geometric* value of the cover read in the `p`-chart (the limit-repair `holoRepr`, whose value
matches `f.toRiemannSphere` and whose value fibre `F⁻¹(coe c)` is the genuine sphere fibre). This
file supplies that hypothesis from the intrinsic local degree `localDeg f (coe c) p` — the *genuine*
ramification multiplicity (= number of regular sheets clustering at `p` over a nearby value, by the
...
* `exists_clusterSplit_at_fibrePoint` — the genuine §5 cluster split at a non-pole fibre preimage,
  ...
  coordinate `η`, its local inverse `s = η⁻¹`, and the cluster sheet preimage property, at the
  genuine `m = (localDeg f (coe c) p).toNat`.

## ⚠ Soundness

The multiplicity is the *genuine* intrinsic local degree `localDeg` (the §17.9
conservation-of-number multiplicity, the one `exists_properMapDegree` is built from), **not** an
ad-hoc number. The geometric value `F = holoRepr ∘ chart.symm` is the genuine limit-repair value of
the cover (its value fibre is the sphere fibre, `holoRepr_pullback_eventuallyEq_toFun`), so the
order is the genuine ramification index. No false/junk field; no custom axiom; no gaps.

## References

* Forster, *Lectures on Riemann Surfaces* (GTM 81), §5 (the normal form `z = wᵐ`, `m` = ramification
  index), §4 (the degree).
* Miranda, *Algebraic Curves and Riemann Surfaces* (1995), §II.4 (the local degree / argument
  principle).
* `Jacobians/ProperMapDegreeSheets.lean` (`exists_sheetDatum_coe`,
  `meromorphicOrderAt_holoRepr_sub_eq`, `localDeg_coe_eq_chartPullback_order`,
...
/-- **The multiplicity bridge.** At a non-pole fibre preimage `p` over `coe c` of a non-constant
cover (`f.div ≠ 0`), set `m := (localDeg f (coe c) p).toNat` (the genuine ramification
multiplicity). Then:
...
This is **exactly** the `hm`/`hord` pair `exists_clusterSplit` / `exists_wpow_normalForm` consume:
the ramified normal form `F = c + ηᵐ` at the genuine multiplicity `m`. The proof mirrors the
per-point order computation inside `exists_sheetDatum_coe` (`ProperMapDegreeSheets.lean`): the
geometric chart-pullback `g = holoRepr ∘ chart.symm` is analytic at the non-pole, its order at the
chart centre is `≠ 0` (it vanishes there, `holoRepr p = c`) and `≠ ⊤` (else the fibre `F⁻¹(coe c)`
would be infinite, contradicting finiteness), and
`localDeg = (meromorphicOrderAt of the toFun pullback) = (analyticOrderAt of the holoRepr
pullback)`.
-/
...
over `coe c` of a non-constant cover (`f.div ≠ 0`), with `m := (localDeg f (coe c) p).toNat` the
genuine multiplicity and `ζ` a primitive `m`-th root, the geometric chart-pullback
`F = f.holoRepr ∘ chart_p.symm` has the §5 normal form `F = c + ηᵐ`, with local inverse `s = η⁻¹`
(`s 0 = chart_p p`), and the cluster sheet points `clusterSheet s ζ w₀ j z = s(ζʲ w₀ z)` are genuine
preimages of `z` under `F`.
...
full-fibre cluster identity `hgeom_fibre` consumes, *with the multiplicity verified to be the
genuine local degree* (not asserted). -/
