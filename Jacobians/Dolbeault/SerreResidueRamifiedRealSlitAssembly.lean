...
`RealSlitClusterSplitData` (`hderiv`/`hmero`/`hcoh`/`hfin_z`/`hreg_z`/`hsheet_diff`) at any slit
value `z` off the branch locus, via `RealSlitClusterSplitData.ofRegularValue`. This file performs
the remaining bookkeeping reduction: it packages the genuinely-remaining **§5 normal-form section
facts** as a single per-slit-value predicate `RealSlitSectionData` and assembles a
`RealCenterClusterFamily` from a slit-wide family of them.
...
* a small slit `Sset` accumulating at `c`, off the branch locus, with a sphere sheet system per
  value;
...
This is the precise irreducible content — the genuine Forster §5 normal-form geometry on a slit near
a ramified centre; everything else (the residue calculus, the conservation-of-number `hcard`, the
off-centre/∞ machinery, the regular-value primitives, the genericity) is proven.

## ⚠ Soundness

`Sset` is a slit accumulating at `c`, off the branch locus (so the regular-value primitives apply at
every `z ∈ Sset`). The §5 section facts are the genuine normal-form geometry — the cluster sections
are genuine local sections of `f.holoRepr` (the §5 atom `exists_clusterSplit_at_fibrePoint`),
distinct at distinct sheets/preimages (the primitive root + T2 separation). No custom axiom; no
unproved obligation on a false statement; no false/junk/circular field. The genuine non-vacuity is
the standard slit-near-`c` regime, where the §5 normal form holds for the cluster arguments
`ζʲ·w₀ z → 0`.
...
/-- **The §5 normal-form section facts at one regular slit value `z`** (the precise residual).
Bundles, for the whole-fibre cluster data `D = realFibreData g hdiv c hnp` and the §5 cluster data
`Cl` on the slit `Sset`, exactly the fields of `RealSlitClusterSplitData.ofRegularValue` that are
NOT discharged by the regular-value primitives:

* `hcs_sec` — the cluster section is a genuine local section of `f.holoRepr` near `z` (the §5
  normal-form section property `f.holoRepr (clusterSection … w) = w`);
* `hcs_np` — the cluster section is a non-pole at `z`;
* `hwithin`/`hcross` — within-cluster injectivity (the §5 straightening uniqueness) and
  cross-cluster separation (distinct preimages give disjoint clusters);
...
/-- **`RealSlitClusterSplitData` from a `RealSlitSectionData` at an off-branch slit value.** Feeds
the §5 section facts together with a sphere sheet system into
`RealSlitClusterSplitData.ofRegularValue` (which discharges the regular-value primitives +
`hsheet_diff`). -/
...
/-- **`RealCenterClusterFamily` from a slit-wide §5 section-data family** (the precise residual
assembly). At a finite pole-value centre `c`, given:

* `Sset`/`hS_acc` — a slit accumulating at `c`, off the branch locus (`hSset_offBranch`), with a
  sphere sheet system per value (`hSsys`);
...
build the per-centre `RealCenterClusterFamily`. Each slit value's `FibreClusterTopology` is
assembled by `RealSlitSectionData.toClusterSplitData` (the regular-value primitives discharged) →
`RealSlitClusterSplitData.toFibreClusterTopology` (the conservation-of-number `hcard` proven
internally) → `RealCenterClusterFamily.ofSlitClusterSplitFamily`.

This isolates the per-centre residual to **exactly** the §5 section facts on the slit (plus the
standard meromorphy bookkeeping `hanalytic`/`hbnd`/`hsplit0` and the slit-near-`c` construction). -/
...
We bundle the entire per-centre residual (the slit + sheet systems + §5 cluster data + bookkeeping +
the per-slit §5 section facts) as one structure `RealCenterSlitSectionData`, and prove the residue
theorem `∑Res = 0` from the obligation "every adapted cover has one such datum at each centre". This
exhibits the residue theorem's precise remaining content as exactly the per-centre §5 normal-form
section geometry on a slit, with EVERYTHING else (the residue calculus, the conservation-of-number
`hcard`, the off-centre/∞ machinery, the regular-value primitives, the genericity) proven. -/

/-- **The bundled per-centre §5-data residual** at a finite pole-value centre `c`. Carries the
whole-fibre non-pole datum, the eventual analyticity + pole-order bound of the geometric trace, a
slit accumulating at `c` off the branch locus with a sphere sheet system per value, the §5 cluster
data of matching multiplicity with its residue split, and the per-slit-value §5 section facts. This
is the precise remaining content the residue theorem needs at one centre. -/
...
/-- **`RealCenterClusterFamily` from a bundled `RealCenterSlitSectionData`.** Feeds the bundled
per-centre §5 data into `RealCenterClusterFamily.ofRegularSlitData`. -/
...
/-- **The single remaining obligation for the residue theorem (real-cover §5-section route).** For
every adapted cover `A`, at each finite pole-value centre `A.cs i`, the bundled per-centre
§5-section datum `RealCenterSlitSectionData` holds. This is the precise remaining content — the
genuine Forster §5 normal-form section geometry on a slit near each ramified centre, with the
regular-value primitives, the conservation-of-number `hcard`, the off-centre/∞ machinery, and the
genericity all proven. -/
...
/-- **`RealCoverClusterGeometry` from `RealCoverSlitSectionGeometry`.** Each per-centre bundled
datum yields a `RealCenterClusterFamily` via `ofCenterSlitSectionData`. -/
...
/-- **The 1-form residue theorem `∑Res = 0` for `α = ω₀·g`, from the per-centre §5-section
...
given the single obligation `RealCoverSlitSectionGeometry` (per centre: a slit accumulating
off-branch + a sphere sheet system per value + the §5 cluster data + the §5 section facts). The
genericity `existsAdaptedFRamified` is proven (axiom-clean), and `ofSlitSectionGeometry` feeds the
§5 data through the proven assembly down to `∑Res = 0`. This is the SOUND well-definedness content
underlying the global `Res : H¹(X,Ω) → ℂ` (Forster 17.3) → §17.5, reduced to exactly the §5
normal-form section geometry. -/
...
(`A.m = 0`), `RealCoverSlitSectionGeometry` holds vacuously, so `∑Res = 0` follows. This confirms
the §5-section obligation is **not** a disguised `False`: in the `m = 0` situation (e.g. all α-poles
over `∞`) the residue theorem closes with no §5 content needed. -/
