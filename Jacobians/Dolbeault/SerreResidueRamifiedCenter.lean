...
/-!
# The residue theorem `∑Res = 0` — admitting *ramified* finite pole-value centres (Miranda
§VIII.3 ramified)
...
*only* to obtain, at each finite pole-value centre `cs i`, the **two facts** the principal-part
close needs:
...
In the *unramified* case (`cs i` off `f`'s branch locus, the `hoff_cs` hypothesis), `Cfull i`
realises both facts through the chain `FibreRegularData → fibreTrace → resAt_traceCoeff'` (the
unramified Lemma 3.2), all of which bake in `deriv ≠ 0` at every fibre point. At a **ramified**
centre the fibre points *are* the ramification points (`deriv = 0`), so that whole route is
structurally impossible — but the two facts (A)/(B) are *still true*: they are exactly the
**ramified Lemma 3.2** (Miranda §VIII.3, (3.1)), now proven as the algebraic `m`-sheet-sum atom
`Jacobians.RamifiedTrace.resAt_laurentTraceCoeff` / `meromorphicAt_laurentTraceCoeff`.
...
field-group (`Cfull`, `hfull_inj`, `hpole_image`, `hnonpole_an`) **replaced** by the two facts
(A)/(B) taken *directly as hypotheses* per centre — exactly the abstraction boundary at which a
ramified centre plugs in. This is a pure refactor of the existing proof (the `Cfull` route is one
way to supply (A)/(B); the ramified atom is another), so it is complete and axiom-clean. The
unramified `…_germ` capstone is recovered as the special case where (A)/(B) come from `Cfull`
(sanity: `…_facts_of_Cfull`).
...
* the **geometric-trace ramified identification** (the one hard analytic piece, Forster §5 local
  normal form `f = wᵐ`): the geometric trace germ `valueChartTrace ω₀ f Φ` near `c` germ-equals the
  algebraic `m`-sheet-sum trace of the per-preimage chart integrands. This is the ramified analogue
  of the unramified `MovingCoherenceDatum.coherent_punctured`; it is supplied as a precise
  hypothesis (`hcoh`). The atom then discharges (A) (`meromorphicAt_laurentTraceCoeff`) and (B)
...
`hcoh` is the genuine geometric content (the manifold cover *is* `z = wᵐ` at a ramification point),
not a disguised residue identity; at an *unramified* centre (`m = 1`) it reduces to the unramified
moving coherence. No `hoff_cs` reappears under another name; no full RR is used (no circularity).
...
* `Jacobians/RamifiedResidueChangeOfVariables.lean` — the 3 consumption sites + the 2 consumed
  facts.
-/
...
direct per-centre hypotheses gives the identical `GlobalTraceData` — the abstraction boundary at
which a ramified centre (whose `Cfull` cannot exist) supplies (A)/(B) from the ramified atom
instead. -/
...
noncomputable def globalTraceData_of_genus0_germ_facts
  ...
  refine
    { L := L
      ...
      hL32 := ?_
      infty_eq := ?_ }
  · -- Finite Lemma 3.2 from fact (B): the centre's trace residue is the pole-fibre
    -- `formFnResidue` sum.
    intro p hp
    rw [hLcenters] at hp
    simp only [Finset.mem_image, Finset.mem_univ, true_and] at hp
...
The unramified capstone supplies the two facts from the per-centre full-fibre moving coherence
`Cfull i`: fact (A) is
`(meromorphicAt_traceCoeff_fibreTrace (Cfull i).D).congr (Cfull.coherent_punctured …)`, fact (B) is
`hres_fin_of_fullFibreCoherence`. We expose this as a sanity lemma confirming `…_facts` is a
...
route). Confirms `globalTraceData_of_genus0_germ_facts` subsumes the unramified `Cfull`-based
discharge: `Cfull i` supplies fact (A) (`hT_mero_cs`, via `coherent_punctured` +
`meromorphicAt_traceCoeff_fibreTrace`) and fact (B) (`hres_fin_cs`, via
`hres_fin_of_fullFibreCoherence`). -/
theorem facts_of_Cfull {Φ : (b : ℂ) → FibreRegularData g f b} {br : Finset ℂ} {m : ℕ}
    {cs : Fin m → ℂ}
    (D : (p : ℂ) → FibreRegularData g f p)
  ...
  · -- (B): the full-fibre coherence residue identity.
    exact hres_fin_of_fullFibreCoherence (poles := poles) D i (Cfull i) (hfull_inj i)
      (hxs_inj (cs i))
      (hpole_image i)
...
`pⱼ` of multiplicities `mⱼ` (`∑ mⱼ = deg`), and the regular-fibre route
(`FibreRegularData`/`fibreTrace`, which bake `deriv ≠ 0`) is structurally impossible. But facts
(A)/(B) still hold — they are exactly the **ramified Lemma 3.2** (Miranda §VIII.3, formula (3.1)),
proven as the algebraic `m`-sheet-sum atom `Jacobians.RamifiedTrace`. We build the parallel ramified
structure here. -/

/-- **Per-fibre ramified data** for the cover `f` over a *ramified* base value `c : ℂ` (the ramified
analogue of `FibreRegularData`). A finite family of preimages `xs : ι → X` over `coe c`, each
carrying a **ramification multiplicity** `mult i ≥ 1` (`= 1` at an unramified point), with the
chart-pullback of `g` meromorphic at each (so the chart integrand of `α = ω₀·g` has an isolated
singularity there). Unlike `FibreRegularData` there is **no `deriv ≠ 0` field** — the preimages may
be ramification points (`deriv = 0`), exactly the case `FibreRegularData` cannot express. The total
multiplicity equals the fibre degree; the per-preimage normal form `f = w^{mult i}` (Forster §5) is
the geometric content the trace identification rests on. -/
structure FibreRamifiedData (g : X → ℂ) (f : MeromorphicFunction X) (c : ℂ) where
  ...
  /-- `g`'s chart-pullback is meromorphic at each preimage (so the chart integrand of `α = ω₀·g` has
  an isolated singularity there). -/
...
— `facts_of_Cfull`). The carried trace germ `T` is the §VIII.3 trace of `α` read in the value chart
at `c` (algebraically: the `m`-sheet-sum trace of the per-preimage chart integrands); `hcoh` is the
geometric identification of `valueChartTrace` with it (the genuine §VIII.3 content), and
`hmero`/`hres` are the atom's outputs. -/
structure RamifiedCenterFacts (ω₀ : HolomorphicOneForms X) (g : X → ℂ) (f : MeromorphicFunction X)
  ...
  T : ℂ → ℂ
  /-- **The geometric-trace ramified identification** (the genuine §VIII.3 content, Forster §5
  normal form `f = wᵐ`): the geometric trace germ equals `T` on a punctured neighbourhood of `c`. -/
  hcoh : valueChartTrace ω₀ f Φ =ᶠ[𝓝[≠] c] T
  /-- **(A)** `T` is meromorphic at `c` (the ramified Fact A — atom
...
/-- **Fact (A) from a `RamifiedCenterFacts`.** The patched trace is meromorphic at `c`: it
germ-equals `valueChartTrace` (patch inert off `c`), which germ-equals `T` (the geometric
identification `hcoh`), which is meromorphic (the atom's Fact A). -/
...
trace of the per-preimage chart integrand's principal part, shifted so its singularity sits at the
value `c`: `T(z) = ∑ⱼ (laurentTraceCoeff sⱼ cⱼ nⱼ mⱼ)(z − c)`. Meromorphy is the atom's
`meromorphicAt_laurentTraceCoeff` (shifted); the residue is the atom's `resAt_laurentTraceCoeff`
summed over the fibre (each preimage's principal part has the same residue as `α`, the geometric
input `hpp_res`). -/
...
(`circleMap_sub_center`, `deriv_circleMap` is centre-independent), so the two limiting contour
integrals coincide. (A clean reusable fact — *not* the geometric normal-form content.) -/
...
/-- **The shifted Laurent trace coefficient** of a per-preimage principal part, singularity at the
value `c`: `(laurentTraceCoeff sⱼ cⱼ nⱼ mⱼ)(z − c)`. Summed over the fibre this is the algebraic
value-chart trace germ `T` of a ramified centre. -/
...
/-- The shifted Laurent trace term is meromorphic at the value `c` (the atom's Fact A, shifted): it
is `laurentTraceCoeff (· − c)`, meromorphic at `c` since `laurentTraceCoeff` is meromorphic at `0`
...
* per-preimage Laurent **principal-part data** `pp` of `chartIntegrand ω₀ g (D.xs i)` whose residue
  at `0` is the geometric residue of `α` at the preimage (`hpp_res` — the principal part captures
  the residue; a true geometric input);
* the **geometric-trace ramified identification** `hcoh`: the geometric trace germ near `c` equals
  the fibre sum `T = ∑ᵢ (mᵢ-sheet trace of ppᵢ shifted to c)` of the per-preimage `mᵢ`-sheet-sum
  traces (the Forster §5 `f = wᵐ` normal form — the one hard analytic piece, supplied as a precise
  hypothesis),

builds the `RamifiedCenterFacts`. Meromorphy (Fact A) and the residue identity (Fact B) are
discharged by the proven atom (`meromorphicAt_ramifiedTraceTerm` / `resAt_ramifiedTraceTerm`),
summed over the mixed-multiplicity fibre. **No `deriv ≠ 0`** is required (the constructor admits
ramification); **no false/circular field** — `hcoh` is the genuine geometric content, not a residue
identity. -/
noncomputable def RamifiedCenterFacts.ofFibreRamified {ω₀ : HolomorphicOneForms X} {g : X → ℂ}
  ...
  hres := by
    -- Residue of the finite fibre sum splits termwise; each term is the atom's per-preimage
    -- residue.
...
fibre-restricted pole-set sum (pure `Finset` re-indexing on the *injective* enumeration `D.xs` of
the pole fibre `F⁻¹(coe c)` — needs only `hD_inj`/`hD_mem`/`hD_surj`, **no** regularity):
...
`GlobalTraceData`/`FormResidueTrace` route the finite-centre Lemma 3.2 through
`fibre : ℂ → FibreTrace` (the unramified model), so they cannot be fed a ramified centre. We bypass
them: the **general** combine `finiteResidueSum_trace_eq_zero` (Miranda §VIII.3 conclusion) takes a
*plain* per-centre residue function `fibreRes : ℂ → ℂ` with `fibreRes p = resAt L.R p` and gives
`∑_p fibreRes p + Res_∞ L.R = 0`. Feeding it the genuine pole-fibre residue sum (= fact (B) at each
centre, principal-part-matched to `resAt L.R`) and the §VIII.3 `∞`-residue identity
(`infty_eq_of_remainderRegular`, the `hcont_int`-free germ-Cauchy) gives `∑Res = 0` directly —
admitting **ramified** centres, where the per-centre facts come from the ramified atom
(`RamifiedCenterFacts`). -/

/-- **The residue theorem `∑Res = 0` from per-centre facts (A)/(B), admitting ramified
centres.** The `hoff_cs`-free residue theorem: instead of the unramified `Cfull`/`FibreTrace` route,
each finite pole-value centre `cs i` supplies the two facts directly:
...
(`meromorphicAt_patched` / `resAt_patched_filter`); an unramified centre via `Cfull`
(`facts_of_Cfull`). The off-centre analyticity `hreg`/`hbnd` and the entire `∞`-group are verbatim.
This routes the finite combine through `finiteResidueSum_trace_eq_zero` (no `FibreTrace`), so **no
centre regularity is required** — `hoff_cs` is gone. -/
theorem residueSum_eq_zero_of_centerFacts
  ...
  have hinfty : resAtInfty L.R L.ρ
    ...
    infty_eq_of_remainderRegular hoff_rem hrem Dinf_full hcoh_full hfullInf_inj xsInf_po
      hpoInf_inj hpoInf_mem hpoInf_surj hpole_image_inf
      (fun k hk => formFnResidue_eq_zero_of_analyticAt ω₀ g _ (hnonpole_inf_an k hk))
  -- Regroup `∑_centers fibreRes p` to `∑_y ∑ formFnResidue` (centre→value re-indexing via
  -- hcenters_cs).
  ...
  -- fibre sums (`hregroup`, `hinfty`), then the total residue (`residueSum_eq_infty_add_finite`) is
  -- the `add_comm` of that.
...
facts (A)/(B) from the ramified atom (and reduces to the unramified `Cfull` route at `m = 1`) —
gives the genus-`0` the residue theorem `∑Res = 0` *without* the per-centre off-branch
genericity `hoff_cs`. The finite centres may be **ramified**. The remaining per-centre obligation is
exactly the geometric-trace identification `RamifiedCenterFacts.hcoh` (the Forster §5 `f = wᵐ`
normal form). -/

/-- **The residue theorem `∑Res = 0` (genus `0`) from per-centre ramified facts —
`hoff_cs`-FREE.** The ramified-capable analogue of
...
theorem residueTheorem_ofRamifiedCenters_genus0
  ...
  residueSum_eq_zero_of_centerFacts (poles := poles) Φ m cs ρ hcs_ball hcs_inj br hreg hbnd
    hcenters_cs
...
conclusion through the new fact-based route — confirming the ramified generalisation is *sound* (not
a disguised weaker/false statement): it is genuinely a superset of the unramified theorem. -/

/-- **The unramified `Cfull` route, recovered via the fact-based ramified theorem (sanity).** Given
the exact unramified inputs of `residueTheorem_of_directGeometry_genus0_germ` (the per-centre
full-fibre `Cfull`, the regular pole sub-fibre `D` with its enumeration fields, and the `∞`-group),
the residue theorem `∑Res = 0` follows through `residueSum_eq_zero_of_centerFacts` fed by
`facts_of_Cfull` — the `m = 1` reduction. This confirms `residueSum_eq_zero_of_centerFacts` subsumes
the unramified close. -/
...
`RamifiedCenterFacts`. Its residue/algebra core (Facts (A)/(B)) is **done** — discharged by the
proven ramified atom in `RamifiedCenterFacts.ofFibreRamified`. The *single* remaining gap, isolated
as a named predicate below, is the geometric-trace identification `RamifiedCenterFacts.hcoh`: that
the geometric trace germ near `c` *is* the algebraic `m`-sheet-sum trace of the per-preimage chart
integrands.
...
`laurentTraceCoeff_eq_sheetSum` — the atom's soundness identity). Its Lean construction is the
ramified analogue of `exists_planar_section` (the unramified local biholomorphism), needing the
local `z = wᵐ` normal form + the analytic branch section `w₀ = z^{1/m}` — the genuine remaining
analytic build. It is *not asserted* without the normal form (no false/circular field, no custom
axiom). -/

/-- **The remaining per-centre ramified obligation.** At a finite pole-value centre `c` of
`α = ω₀·g` (with `g`'s chart-pullback meromorphic at each preimage), there is a
`RamifiedCenterFacts` — equivalently (by `RamifiedCenterFacts.ofFibreRamified`), a ramified fibre
datum + per-preimage Laurent principal-part data + the geometric-trace identification `hcoh`.

**Soundness (non-false).** Genuinely achievable: enumerate the pole fibre `F⁻¹(coe c)` with
multiplicities (Forster §5), take the Laurent principal part of each `chartIntegrand ω₀ g pⱼ` (a
true Laurent expansion), and identify the geometric trace germ with the `m`-sheet sum via the local
`z = wᵐ` normal form (`laurentTraceCoeff_eq_sheetSum`, the atom's soundness identity). Hence a TRUE
existential — the precise statement of "the §VIII.3 ramified trace exists at `c`", the ramified
analogue of the unramified moving coherence `MovingCoherenceDatum`. -/
...
/-- **The residue theorem `∑Res = 0` (genus `0`) modulo the per-centre ramified obligation,
`hoff_cs`-FREE.** The cleanest statement of the elimination: given `ExistsRamifiedCenterFacts` at
every finite pole-value centre (the *only* remaining per-centre gap — the geometric `z = wᵐ`
identification, Forster §5, mathematically true) plus the off-centre analyticity and the `∞`-group,
the residue-theorem assembly holds with **no** `hoff_cs`. The finite centres may be ramified. -/
