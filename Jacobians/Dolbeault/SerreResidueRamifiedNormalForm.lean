...
contradictory* for `m > 1`. (An earlier version of `RamifiedSheetData` demanded exactly this —
making it a disguised `False` at every genuine ramification centre, satisfiable only at `m = 1`.
...
minus a ray (concretely `{z | z − c ∈ slitPlane}`, on which `w₀ z = (z − c)^{1/m}` via
`Complex.cpow`). On the slit the geometry is exactly the unramified `m`-sheet picture, and the
proven atom applies. But both the geometric trace `valueChartTrace` and the algebraic trace are
**single-valued analytic on the *full* punctured neighbourhood**: `valueChartTrace` because it is
the *symmetric* sum over the `m`-point fibre (a set — independent of the sheet labelling; Miranda
(3.1) makes it meromorphic at `c`), and the algebraic trace `ramifiedTraceTerm + Rem` by
construction. Two functions meromorphic at `c` that agree on a slit accumulating at `c` agree on all
of `𝓝[≠] c` by the **identity theorem** (`MeromorphicAt.frequently_eq_iff_eventuallyEq`). That is
how `hcoh` is *derived* (never asserted).

## The geometry on the slit (Miranda §VIII.3, Forster §5)

On the slit, with the centered chart `w = chart_p` at `p`, the cover reads `z − c = (w − w_p)ᵐ` and
the fibre splits into the `m` sheets `w = w_p + ζʲ·w₀(z)` (`ζ` a primitive `m`-th root of unity).
The value-chart trace over those sheets is the `m`-sheet sum
...
proven roots-of-unity collapse (`Jacobians.RamifiedTrace.laurentTraceCoeff_eq_sheetSum`) this
`m`-sheet sum, applied to the **Laurent principal part** of `h` at `w_p`, equals the closed-form
`laurentTraceCoeff (z − c)` — exactly the algebraic `ramifiedTraceTerm` whose residue is the
upstairs residue (`resAt_laurentTraceCoeff`). The holomorphic remainder of `h` contributes a
holomorphic trace `Rem` (single-valued at `c` by the symmetric-function descent), which the residue
does not see.

## What this file delivers

* **`RamifiedSheetData`** (the genuine geometric input): at one preimage `p`, the multiplicity `m`,
  a primitive `m`-th root `ζ`, a **slit** `S` accumulating at `c`, a holomorphic branch `w₀` of
  `(z − c)^{1/m}` **on `S`** (the Forster §5 analytic atom — the slit `m`-th root, which exists for
  any `m`), the per-preimage Laurent principal-part data + the **single-valued** analytic remainder
  trace `Rem`, the geometric identification `hgeom_slit` (on the slit), and — the genuine new
  content — the **single-valued meromorphy of the symmetric trace sum** `hvct_mero` at `c`. All
  supplied as *data*, none asserted as a free lemma.
...
* **`RamifiedSheetData.traceFull` / `hcoh`** (the genuine new analytic content): the single-valued
  ...
  `m`-sheet sum (the atom, #1), hence equal to `valueChartTrace` on `𝓝[≠] c` by the identity
  theorem.

* **`RamifiedCenterFacts.ofSheetData`** (the sound capstone): assembles a `RamifiedCenterFacts`
  whose carried trace `T` is the single-valued `ramifiedTraceTerm + Rem`, with `hcoh` derived via
  the slit + identity theorem and `hmero`/`hres` via the proven atom. Single-preimage form.

## ⚠ Soundness

* **No monodromy contradiction.** `w₀` is required analytic only on the *slit* `S`; the structure is
  satisfiable for `m > 1` (`ramifiedSheetData_sqrt`, an explicit `m = 2` witness with the genuine
  slit branch `√(z − c)`), not just `m = 1`.
* `T = ramifiedTraceTerm + Rem` is single-valued meromorphic at `c`;
  `hcoh : valueChartTrace =ᶠ[𝓝[≠] c] T` is **derived** (identity theorem from agreement on the
  slit), never asserted. `hmero`/`hres` come from the atom; `Rem` (analytic) contributes residue
  `0`.
...
* Miranda, *Algebraic Curves and Riemann Surfaces* (1995), §VIII.3, (3.1) (Tr single-valued
  meromorphic in `z`), Lemma 3.2 ramified case.
...
The mechanism by which a *slit* identity globalises: two functions meromorphic at `c` that agree on
a set accumulating at `c` agree on the whole punctured neighbourhood (Mathlib's meromorphic identity
...
/-- **Identity theorem on a slit.** If `f` and `g` are meromorphic at `c` and agree on a set `S`
that accumulates at `c` (`∃ᶠ z in 𝓝[≠] c, z ∈ S` — true for a slit), then `f =ᶠ[𝓝[≠] c] g`. This is
the engine that turns the slit-only branch identity into a punctured-neighbourhood germ-equality (so
the multivalued `m`-th root is needed only on the slit, dodging the monodromy obstruction). -/
...
§5 local normal-form geometry as *data* (the ramified analogue of the `exists_planar_section`
output), **with the monodromy obstruction respected**: the `m`-th-root branch `w₀` lives on a *slit*
`S`, not on all of `𝓝[≠] c`. The data also carries the single-valued algebraic trace ingredients
(the Laurent principal part of the chart integrand, and the single-valued analytic remainder trace
`Rem`) plus the genuine new content — the single-valued meromorphy of the symmetric trace sum
`valueChartTrace` at `c`. Everything downstream (`T`, `hcoh`, the residue/meromorphy) is *derived*.
-/

/-- **Forster §5 slit sheet data at one preimage.** At a preimage `p` of multiplicity `m ≥ 1` over
the centre `c`, with chart coordinate `wp := chart_p p`:

* `ζ` a primitive `m`-th root of unity;
* a **slit** `S ⊆ ℂ` accumulating at `c` (`hS_acc`), with a holomorphic branch `w₀` of
  `(z − c)^{1/m}` **on `S`** — nonvanishing (`hw₀_ne`), an `m`-th root (`hw₀_pow`), with the
  chain-rule derivative (`hw₀_deriv`). This is the **Forster §5 analytic atom**, restricted to the
  slit where it exists for *any* `m` (no monodromy contradiction);
* per-preimage Laurent **principal-part data** of the chart integrand `h := chartIntegrand ω₀ g p`:
  degree `ppN`, coefficients `ppb`, analytic remainder `ppR` (`hppR_an`), with the split holding at
  the sheet points along the slit (`hpp_split_sheet`). `hppN_res` records that the principal part
  carries the upstairs residue (a true fact about the Laurent expansion at `p`, *not* the downstairs
  residue);
...
* `hvct_mero` — the genuine new content (Miranda (3.1)): the symmetric trace sum
  `valueChartTrace ω₀ f Φ` (single-valued by construction — a *set* sum over the fibre) is
  **meromorphic at `c`**.

Supplied as *data* (as the unramified `MovingCoherenceDatum` is), never asserted as a free lemma;
the single-valued trace `T`, the punctured germ-equality `hcoh`, the residue and meromorphy are all
...
structure RamifiedSheetData (ω₀ : HolomorphicOneForms X) (g : X → ℂ) (f : MeromorphicFunction X)
  ...
  /-- The chain-rule derivative on the slit: `w₀'(z) = (1/m) w₀(z)^{1−m}` (from
  `m w₀^{m−1} w₀' = 1`). -/
  hw₀_deriv : ∀ z ∈ S, deriv w₀ z = (m : ℂ)⁻¹ * w₀ z ^ (1 - (m : ℤ))
  /-- `g`'s chart-pullback is meromorphic at `p`'s chart centre (so the chart integrand of
  `α = ω₀·g` has an isolated singularity — true since `p` is an isolated pole of `α`). -/
  ...
  /-- **The principal-part split at the sheet points** (true by
  `exists_principalPart_meromorphicAt`): on the slit, the chart integrand decomposes as principal
  part `negTail` plus analytic remainder `ppR` at each of the `m` sheet arguments `wp + ζʲ w₀ z`. -/
  ...
  trace is the `m`-sheet sum of the chart integrand of `α = ω₀·g` at `p`, read along
  `w = wp + ζʲ w₀`. -/
  hgeom_slit : ∀ z ∈ S, valueChartTrace ω₀ f Φ z = ∑ j ∈ Finset.range m,
    chartIntegrand ω₀ g p ((chartAt ℂ p) p + ζ ^ j * w₀ z)
      * deriv (fun ζz => (chartAt ℂ p) p + ζ ^ j * w₀ ζz) z
  /-- **Single-valued meromorphy of the symmetric trace sum** (the genuine new content, Miranda
  (3.1)): `valueChartTrace ω₀ f Φ` — single-valued by construction (a sum over the *fibre set*,
  independent of sheet labels) — is meromorphic at `c`. -/
...
`W ↦ ∑_i cf_i (W − wp)^{n_i}`, its `m`-sheet sum along `w = wp + ζʲ w₀(z)` is exactly the
closed-form `laurentTraceCoeff (z − c) = ramifiedTraceTerm`, by the proven roots-of-unity collapse
...
/-- **The geometric `m`-sheet sum of a Laurent-polynomial integrand is `ramifiedTraceTerm`.** For
the Laurent polynomial integrand `hpoly W := ∑_{i ∈ s} cf_i (W − wp)^{n_i}` and the branch `w₀` at
`z` (`w₀ z ≠ 0`, `w₀(z)ᵐ = z − c`, `w₀'(z) = (1/m) w₀(z)^{1−m}`, `ζ` a primitive `m`-th root), the
...
(`ramifiedTraceTerm` the `m`-sheet-sum trace of the integrand's Laurent **principal part**, `Rem`
the single-valued analytic remainder trace). `T` is single-valued and meromorphic at `c` *by
...
meromorphic at `c` and the slit accumulates at `c`, the identity theorem upgrades the slit identity
to the punctured germ-equality `hcoh` — without ever needing the multivalued branch off the slit. -/
...
Laurent principal part (`ramifiedTraceTerm`, a single-valued meromorphic function of `z − c`) plus
the single-valued analytic remainder trace `Rem`. This is single-valued and meromorphic at `c` by
...
trace `valueChartTrace` equals the single-valued algebraic trace `T = ramifiedTraceTerm + Rem`.
Proof: `hgeom_slit` (geometric = `m`-sheet sum of the chart integrand), then the principal-part
split at the sheet points (`hpp_split_sheet`), the proven atom (`ramifiedSheetSum_laurentPoly`,
turning the principal-part sheet sum into `ramifiedTraceTerm`), and `hRem_slit` (the remainder sheet
sum is `Rem`). -/
...
/-- The single-valued trace `T` is **meromorphic at `c`** by construction: `ramifiedTraceTerm` is
the atom's shifted Laurent term (`meromorphicAt_ramifiedTraceTerm`) and `Rem` is analytic at `c`. -/
...
`resAt T c = formFnResidue ω₀ g p`. The residue of `ramifiedTraceTerm` is the principal part's
residue (`resAt_ramifiedTraceTerm` + the data field `hppN_res`), and `Rem` (analytic) adds `0`. -/
theorem resAt_traceFull (S : RamifiedSheetData ω₀ g f Φ c) :
  ...
  rw [show (fun z => ramifiedTraceTerm (Finset.Icc 1 S.ppN) S.ppb (fun k => -(k : ℤ)) S.m c z
    ...
    resAt_add
      (meromorphicAt_ramifiedTraceTerm (Finset.Icc 1 S.ppN) S.ppb (fun k => -(k : ℤ))
        S.m c).holoPunctured
...
`T := traceFull` is the single-valued `ramifiedTraceTerm (principal part) + Rem` (NOT the
multivalued sheet sum), so `hcoh : valueChartTrace =ᶠ[𝓝[≠] c] T` is the genuine punctured
germ-equality, *derived* via the slit identity + the identity theorem; `hmero`/`hres` come from the
proven atom. -/

/-- **`RamifiedCenterFacts` from a single-preimage `RamifiedSheetData`** (the SOUND ramified centre
provider). Given the Forster §5 slit sheet data `S` at a ramification preimage `p` of multiplicity
`m` over `c`, and that `p` is the *unique* pole of `α = ω₀·g` in the fibre `F⁻¹(coe c)` (`hp_pole`,
...
— TRUE, no monodromy contradiction); `hmero`/`hres` are `meromorphicAt_traceFull`/`resAt_traceFull`
(the proven atom). **No `deriv ≠ 0`** (admits ramification); **no false/circular field**. -/
...
/-! ### Soundness sanity — the carried trace is single-valued; the slit identity recovers the
geometry

The carried trace `T = traceFull` is, by construction, the **single-valued** function
`ramifiedTraceTerm (principal part) + Rem` (a meromorphic function of `z − c`), *not* the
multivalued sheet sum. It coincides with the geometric `m`-sheet sum exactly on the slit `S`
(`eqOn_traceFull_slit`), and that slit agreement is what the identity theorem promotes to `hcoh`. At
an *unramified* preimage (`m = 1`) the slit can be taken to be the full punctured neighbourhood and
the construction degenerates to the unramified single-section identification (witnessed at `m = 1`
by `ramifiedSheetData_zero`). We record the single-valued definitional shape as a sanity check. -/
...
/-! ### Non-vacuity of `RamifiedSheetData` — inhabitable for **any** `m` (the monodromy fix
verified)

`RamifiedSheetData`'s fields are *jointly satisfiable for every `m ≥ 1`*, so
`RamifiedCenterFacts. ofSheetData` is **not** vacuous, and — crucially — the structure is **not a
disguised `False` at a ramified centre** (the bug it replaces). Witness: the zero numerator `g ≡ 0`
(so `α = ω₀·g = 0`), the empty fibre selection (`valueChartTrace ≡ 0`), with the genuine **slit
`m`-th-root branch** `w₀ z = (z − c)^{1/m}` (`Complex.cpow`) on the slit
`S = {z | z − c ∈ slitPlane}` — which exists for *any* `m` (the whole point of the slit). At `m = 2`
this is the genuine `√(z − c)` branch on the slit-plane, demonstrating the structure is inhabited at
a *ramified* multiplicity. Every other field is `0 = 0` by direct computation (`g ≡ 0`), and the
branch fields are the standard `cpow` identities. -/

/-- **`RamifiedSheetData` is inhabitable for every `m ≥ 1`** (non-vacuity / soundness witness — the
monodromy fix). For the zero numerator `g ≡ 0`, the empty fibre selection, any centre `c`, point
`p`, multiplicity `m ≥ 1`, and primitive `m`-th root `ζ`, the slit `S = {z | z − c ∈ slitPlane}`
with the `cpow` branch `w₀ z = (z − c)^{1/m}` is a valid `RamifiedSheetData`. The branch exists on
the slit for **any** `m` (no monodromy contradiction), so the structure is inhabited at genuinely
ramified multiplicities — e.g. `m = 2`, the `√(z − c)` branch (`ramifiedSheetData_sqrt`). -/
noncomputable def ramifiedSheetData_slit (ω₀ : HolomorphicOneForms X) (f : MeromorphicFunction X)
    ...
    RamifiedSheetData ω₀ (fun _ => (0 : ℂ)) f
  ...
  hgeom_slit := by
    intro z _
    -- `valueChartTrace = 0` (empty selection), and the RHS is the `m`-sheet sum of
    -- `chartIntegrand 0 = 0`.
...
/-- **`RamifiedSheetData` is inhabited at the ramified multiplicity `m = 2`** — the genuine
`√(z − c)` slit branch. This is the decisive soundness witness: the corrected structure is
satisfiable for `m > 1` (the old `∀ᶠ in 𝓝[≠] c` branch demand was a disguised `False` here by
monodromy). Uses the primitive square root `ζ = −1` and the `cpow` branch `√(z − c)` on the slit
plane. -/
...
/-- **`RamifiedSheetData` is inhabited at `m = 1`** (the unramified reduction — `ζ = 1`). Confirms
the construction degenerates correctly; here the slit branch is `(z − c)^{1/1} = z − c`. -/
...
/-! ### Wiring to `ExistsRamifiedCenterFacts` (the precise remaining obligation, now Forster §5
data)
...
(at a single ramified preimage that is the unique pole in its fibre).
`residueTheorem_ofRamifiedCenters_genus0_mod` then gives `hoff_cs`-free the residue-theorem assembly
from per-centre `RamifiedSheetData`. The genuine remaining analytic content is exactly the
`RamifiedSheetData` fields: the slit `m`-th-root branch `w₀`, the geometric trace identification
`hgeom_slit`, the single-valued analytic remainder trace `Rem`, and the single-valued meromorphy
`hvct_mero` — all true (Forster §5 / Miranda (3.1)), supplied as data, none asserted as a free
lemma. -/

/-- **`ExistsRamifiedCenterFacts` from a single-preimage `RamifiedSheetData`.**  Packaging the sound
constructor `RamifiedCenterFacts.ofSheetData` as the named per-centre obligation: the Forster §5
sheet data at a ramified preimage `p` (the unique pole in the fibre `F⁻¹(coe c)`) inhabits
...
/-- **The residue theorem `∑Res = 0` (genus `0`) from per-centre Forster §5 sheet data —
...
identification (Forster §5), supplied as data. -/
theorem residueTheorem_ofSheetData_genus0 {ω₀ : HolomorphicOneForms X} {g : X → ℂ}
    ...
    -- The per-centre Forster §5 sheet data (the only genuine remaining content; admits
    -- ramification).
    (Sf : ∀ i, RamifiedSheetData ω₀ g f Φ (cs i))
    (hp_pole : ∀ i, (Sf i).p ∈ poles)
    (hp_fibre : ∀ i, f.toRiemannSphere (Sf i).p = ((cs i : ℂ) : RiemannSphere))
    (hp_unique : ∀ i, ∀ a ∈ poles,
      f.toRiemannSphere a = ((cs i : ℂ) : RiemannSphere) → a = (Sf i).p)
  ...
  residueTheorem_ofRamifiedCenters_genus0_mod (poles := poles) Φ m cs ρ hcs_ball hcs_inj br
    hreg hbnd
    hcenters_cs
