/-
  Dolbeault ladder — Čech finiteness node, BASE LAYER: the sup-norm Montel model types and the
  abstract finiteness lemmas (Forster 14.9).

  This file holds the model TYPES (`DiskOverlapData`, `Coboundaries`) and the abstract spine
  (`rhoRaw`/`rhoRaw_compact`, the cocycle spaces, `finiteDimensional_supH1`, `leray_surjective`,
  the trivial acyclic model, and the acyclic `exists_cechModel_of_subsingleton`).  The keystone
  re-export `exists_cechModel` (and its consumers `cechH1_linearEquiv_supH1`,
  `finiteDimensional_cechH1_wired`) lives in the thin top layer `CechFinitenessWiring`, which imports
  this file together with the proven general term `CechFinitenessDtwist.exists_cechModel_general`.
  The split exists purely to break the model-types ↔ general-term import cycle (the general term's
  artificial model imports these types); there is NO new mathematics here.
  ...
  where the model is CONSTRUCTED — the keystone `exists_cechModel` (now PROVEN, in
  `CechFinitenessWiring`, re-exporting `exists_cechModel_general`). The disk side of that obligation
  is the proven full-disk ∂̄-solvability `DbarDiskCohomology.dbar_solvable_ball` and the holomorphic
  re-splitting `DbarDiskCohomology.dbar_holo_splitting_ball` (both sorry-free, axiom-clean).

  ALSO ASSEMBLED HERE (sorry-free): the trivial acyclic model and `exists_cechModel_of_subsingleton`
  (the model existential for a subsingleton germ-class `H¹`).  STEPS 3 (`ρ` compact), 4 (Leray
  surjectivity), and 5 (the reduction application) are fully proven; everything in this file is
