...
/-- `∂̄` is additive (`differential` is additive via `mfderiv`; `proj01` is linear). TRUE; the bounded
first atom of the Dolbeault comparison. -/
theorem dbar_add (u v : SmoothCFunctions X) : dbar (u + v) = dbar u + dbar v := by
  sorry

/-- `∂̄` is ℝ-homogeneous (same reason). TRUE; bounded. -/
