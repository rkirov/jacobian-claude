...
* `hglue_fin` — `valueChartTrace` germ-equals the pole sub-fibre trace `(fibreReg hac (cs i))` off
  each finite pole-value;
* `hglue_inf` — `recipCoeff (valueChartTrace)` germ-equals the `∞`-fibre trace off `0`;
* `hreg` — at each value off the centres, `valueChartTrace` germ-equals a fixed *regular* fibre
  trace;
...
*single fixed* fibre. That is Miranda's "the trace `Tr_F α` is a single (mero)morphic function" read
in the fibre language.
...
fibre points germ-match index-by-index near a base value, their trace coefficients germ-agree
there), and the **genus-`0` `∞`-vanishing as a sphere-form corollary**
(`coeffAt_eq_zero_of_sphereForm`).
...
* `coherentTraceSelection_empty` / `residueSum_eq_zero_of_coherentSelection_holomorphic` —
  non-vacuity (the empty-pole case), confirming the structure is honest (not a disguised `False`).
...
germ at the base (the `coeff i` is a *fixed* function, and `deriv` and composition are germ-local).
We record the per-summand statement and the assembled finite-sum statement; both are pure
germ-locality (no geometry), and they are what the proved section identification feeds into. -/
...
theorem traceSummand_eventuallyEq_of_section_eventuallyEq (c : ℂ → ℂ) {s₁ s₂ : ℂ → ℂ} {b : ℂ}
  ...
  -- `deriv s₁ =ᶠ[𝓝 b] deriv s₂`: `deriv` is germ-local (a pointwise statement on the
  -- neighbourhood).
...
chart pullbacks through the fibre points (not just the `exists_planar_section`-chosen sheets baked
into `fibreTrace`). This is the reformulation a coherent-selection builder uses: the manifold local
sections (`exists_holo_localInverse` / `LocalSheetSystem`), read in charts, are such right-inverses,
so the geometric trace built from them has the *same germ* as the planar fibre trace. -/

/-- **The local fibre trace coefficient via arbitrary right-inverse sections.**  Let `D :
FibreRegularData g f b`, and let `rinv : D.ι → ℂ →
ℂ` be a family of continuous right-inverses of the chart pullbacks of `f.holoRepr` through the
fibre points: for each `i`, `rinv
i b = chart_{xs i}(xs i)`, `rinv i` continuous at `b`, and `f.holoRepr (chart_{xs i}.symm (rinv i
...
The genus-`0` `∞`-vanishing input `R₀ 0 = 0` is, conceptually, "the holomorphic remainder
`Tr_F α − L.R` is a holomorphic `1`-form on `ℂℙ¹`, hence `0` (`H⁰(ℂℙ¹, Ω) = 0`), hence its
`∞`-coefficient is `0`". We record the reusable consequence: *any* holomorphic `1`-form on
`RiemannSphere` has identically zero chart coefficient `coeffAt` at every point (it is the zero
section by `ProjectiveLine.holomorphicOneForm_eq_zero`). Once the remainder is packaged as such a
sphere form, its value-chart coefficient (and via the reciprocal-chart Jacobian, `R₀`) vanishes —
discharging the genus-`0` field of `CoherentTraceSelection`. -/
...
is the §VIII.3 monodromy: the geometric trace `valueChartTrace ω₀ f Φ` (which at each base value
`b'` re-selects the fibre `Φ b'`) germ-equals a **single fixed** local fibre trace near each value —
the finite pole-values (against the pole sub-fibre `fibreReg hac`), `∞`, and every regular value.
Plus junk-freeness and the genus-`0` `∞`-vanishing. -/
...
* `hreg` — at each value off the centres, germ-equality with a fixed *regular* fibre trace (with
  `g`'s chart-pullback analytic at each of its fibre points);
...
structure CoherentTraceSelection (ω₀ : HolomorphicOneForms X) (g : X → ℂ)
  ...
  /-- **Off-exceptional coherence.** At each value off the centres, the geometric trace germ-equals
  a fixed regular fibre trace (with `g`'s chart-pullback analytic at each fibre point). -/
...
The `CoherentTraceSelection` obligation is *satisfiable*, not a disguised `False`: for the **empty
pole set** (`α = ω₀·g` globally holomorphic) the **empty fibre selection**
`Φ p := emptyFibreRegularData g f p` gives `valueChartTrace ω₀ f Φ ≡ 0` (an empty fibre sum), with
all the deep germ-coherence inputs (finite/∞ glue, off-exceptional coherence, junk-freeness) vacuous
over the empty centre set, the `∞`-glue `recipCoeff 0 ≡ 0` matching the empty `∞`-trace, and the
genus-`0` continuation `R₀ ≡ 0` vanishing. This confirms the structure produces a genuine
`∑Res = 0`. -/

/-- **A coherent fibre selection for the empty pole set.** The empty fibre selection (no fibre
points over any value, `valueChartTrace ≡ 0`) is a coherent selection for the globally-holomorphic
case: no finite centres (every glue/junk-free field vacuous), the empty `∞`-fibre
(`recipCoeff 0 ≡ 0`), and the vanishing genus-`0` continuation `R₀ ≡ 0`. The honest non-vacuity
witness. -/
