...
sphere-specific dissolution of the base monodromy: the only nontrivial transition is per-sheet on
the *total* space (handled by the proven `localRep_eq_transition_mul_self` inside the per-sheet
bridge), never on the sphere base.
...
/-- **Sphere finite-base trivialization agreement.** The tangent-bundle trivializations at two
finite points `coe a` and `coe b` of `RiemannSphere` are equal (both are the trivialization built
from the single global affine chart `chartCoe = chartAt ℂ (coe ·)`). -/
...
functional read in the *fixed* `coe a`-frame equals the one read in the *self* `coe b`-frame, for
any covector `φ` at `coe b`: the two trivializations coincide (`trivializationAt_coe_eq`). This
dissolves the base "monodromy" — there is no genuine frame transition on the sphere base. -/
...
point `x` equals `g x • ω₀.toFun x`. Hence the bundle per-sheet summand of `αBr` factors as
`g (s y)` times the holomorphic per-sheet summand of `ω₀`. -/

/-- **`g`-weight pull-out for the bundle summand.**  If `αBr.toFun (s y) = g (s y) • ω₀.toFun (s y)`
(the form `αBr` is `ω₀·g` at the fibre point `s y`), then the bundle per-sheet covector of `αBr` at
the affine unit tangent is `g (s y)` times that of `ω₀`:
...
nonconstant cover, `coe z` a regular value with a local sheet system `S`, and `αBr` a holomorphic
form with `αBr.toFun (S.sheet i (coe z)) = g (S.sheet i (coe z)) • ω₀.toFun (S.sheet i (coe z))` at
each fibre point. Then for any base point `coe b₀`,
...
per-sheet summand reduce to the *self-chart intrinsic* summand `g (P)·localRep ω₀ P P·deriv(…)`.
Hence they are equal, term by term over the sheet index — *no reindexing*, the two sums share the
index `Fin S.n`. -/
...
>
  `chartIntegrand ω₀ g (s (coe z)) (chart_{s(coe z)}(s (coe z)))
   ·deriv (fun w => chart_{s(coe z)}(s (coe w))) z
>    = g (s (coe z))·sheetPullback ω₀ s (coe z) 1`.

Both equal `g (s(coe z))·localRep ω₀ (s(coe z))(s(coe z))·deriv(chart_{s(coe z)} ∘ s(coe ·)) z`: the
LHS by `movingSummand_eq_selfChart` (the `dz`-Jacobian cancellation at the self chart
`xs = s(coe z)`), the RHS by the linchpin `sheetPullback_one_eq_coeffAt_mul_deriv` +
`coeffAt_chartCenter`. -/
theorem movingSummand_eq_g_sheetPullback (ω₀ : HolomorphicOneForms X) (g : X → ℂ)
    (s : RiemannSphere → X) {z : ℂ}
    (hs : MDifferentiableAt 𝓘(ℂ) 𝓘(ℂ) s (((z : ℂ) : RiemannSphere))) :
    chartIntegrand ω₀ g (s (((z : ℂ) : RiemannSphere)))
        ((chartAt ℂ (s (((z : ℂ) : RiemannSphere)))) (s (((z : ℂ) : RiemannSphere))))
        * deriv (fun w => (chartAt ℂ (s (((z : ℂ) : RiemannSphere))))
            (s (((w : ℂ) : RiemannSphere)))) z
      = g (s (((z : ℂ) : RiemannSphere)))
        * sheetPullback ω₀ s (((z : ℂ) : RiemannSphere)) (1 : ℂ) := by
  -- RHS via the linchpin (read in the moving `s(coe z)`-chart, source chart `chartCoe` on the
  -- sphere).
  rw [FormTraceSheet.sheetPullback_one_eq_coeffAt_mul_deriv ω₀ s hs]
  -- The source sphere chart is `chartCoe`: `(chartAt (coe z)).symm w = coe w`,
  -- `chartCoe (coe z) = z`.
...
with `αBr.toFun (S.sheet i (coe z)) = g (S.sheet i (coe z)) • ω₀.toFun (S.sheet i (coe z))`, and
`D := FibreRegularData.ofSphereSheetSystem S hderiv hmero` the sphere-sheet fibre over `z`. Then for
any base point `coe b₀`,
...
Lifting the pointwise identity to the germ equality `hbridgeBr` of
`patchedTraceSelection_ofBundleBranch`. On the punctured neighbourhood `𝓝[≠] b₀`, eventually each
`z` is a regular value carrying a sphere sheet system `S_z`, with (i) the geometric trace agreeing
with the sphere-sheet planar fibre trace at `z` (the `Creg`-coherence, available from the
moving-fibre machinery), and (ii) `αBr.toFun = g·ω₀.toFun` at the fibre points of `S_z` (the form
`αBr` is `ω₀·g` near the fibre). At each such `z` the pointwise identity
`fibreTrace_traceCoeff_eq_traceLocalCoeff` finishes. -/

/-- **The bundle-trace germ bridge `hbridgeBr` from the eventual sphere-sheet coherence.** Suppose
on `𝓝[≠] b₀`, eventually each `z` carries a sphere sheet system `S_z` of `F = f.toRiemannSphere` at
`coe z` (with regular fibre — `hderiv`, `hmero`) such that the geometric trace agrees with the
sphere-sheet fibre trace at `z` and `αBr` is `ω₀·g` at the fibre points:

> eventually:
  `valueChartTrace ω₀ f Φ z = (fibreTrace ω₀ f (ofSphereSheetSystem S_z …)).traceCoeff z`
> and
  `∀ i, αBr.toFun (S_z.sheet i (coe z)) = g (S_z.sheet i (coe z)) • ω₀.toFun (S_z.sheet i (coe z))`.

Then the bundle-trace germ bridge holds:

> `valueChartTrace ω₀ f Φ =ᶠ[𝓝[≠] b₀] z ↦ traceLocalCoeff (traceFun F αBr) (coe b₀) (coe z)`.

This is exactly the `hbridgeBr` input of `patchedTraceSelection_ofBundleBranch`, reduced to the
eventual regular-value sphere-sheet coherence (the standard §VIII.3 residual — *no*
monodromy/Puiseux frame). -/
...
Part (2) of `hevBr` — the geometric trace agrees with the sphere-sheet fibre trace at `z` — is
*exactly* the regular-value coherence already available from the `Sreg`/`Creg` machinery: the
`MovingCoherenceDatum.ofSphereSheetSystemCanon` datum has `.D = ofSphereSheetSystem (Sreg z) …`, so
its `.coherent` (taken at `z` via `self_of_nhds`) gives the identity. Hence the bundle-branch path
needs *no new* geometric trace input at branch values; the same regular-value sphere data that
handles every regular value discharges it. -/
...
The per-branch eventual sphere coherence `hevBr` is *entirely* discharged from the regular-value
data already supplied to `patchedTraceSelection_ofBundleBranch` (the `Sreg` family with its
canonical-fibre conditions) PLUS the per-branch `αBr`-agreement-at-fibre-points (the form
`αBr b₀ = ω₀·g` near the fibre — the standard Mittag-Leffler/cutoff residual). No new geometric
input at branch values: the SAME `Sreg z` data that handles every regular value, applied to regular
`z` near `b₀`, gives both the sphere sheet system and the geometric-trace coherence.
Eventually-`z`-regular near `b₀` is the finite-bad-set avoidance (`br ∪ image cs` is finite, `b₀`
isolated). -/
...
* `hαBrAgree` — `αBr b₀ = ω₀·g` at the fibre points `(Sreg z hz).sheet i (coe z)` for `z` near `b₀`
  (the only genuine per-branch residual: the local-holomorphic form agreeing with `ω₀·g` near the
  fibre).
...
theorem hevBr_of_regularData (ω₀ : HolomorphicOneForms X) (g : X → ℂ) (f : MeromorphicFunction X)
  ...
  -- Eventually `z ∉ image cs ∪ br` on `𝓝[≠] b₀`: the finite bad set minus `{b₀}` is closed and
  -- avoids `b₀`.
...
/-! ### The residue theorem `∑Res = 0` with the branch input reduced to the `αBr`-agreement
only

The cleanest close: wiring `hevBr_of_regularData` into `residueSum_eq_zero_ofBundleBranchCoherence`,
...
theorem residueSum_eq_zero_ofBundleBranchAgree (hac : AdaptedCover ω₀ g f poles)
    ...
    (hbrBr : ∀ b₀ ∈ br, b₀ ∉ Finset.univ.image cs →
      ((b₀ : ℂ) : RiemannSphere) ∈ branchLocus f.toRiemannSphere)
    -- The single per-branch residual: the local-holomorphic form `αBr b₀ = ω₀·g` at the fibre
    -- points.
