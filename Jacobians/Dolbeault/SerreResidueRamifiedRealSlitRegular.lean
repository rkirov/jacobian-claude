...
`RealSlitClusterSplitData.toFibreClusterTopology` (feeding
`FibreClusterTopology.ofClusterSplitData`). The fields of `RealSlitClusterSplitData` fall into two
groups:

* the **regular-value primitives** — the sphere sheet system `S` and its regularity
  `hderiv`/`hmero`, the coherence `hcoh`, and the conservation-of-number cover-degree readings
  `hfin_z`/`hreg_z`; these are GENUINELY mechanical at a value `z` *off the branch locus* (a regular
  value of the cover) and are discharged here from the proven atoms;
* the **§5 normal-form section facts** — `hcs_sec`/`hcs_np`/`hwithin`/`hcross`/`hsrc`/`hsheet_diff`;
  these are the genuine Forster §5 normal-form geometry (the cluster section is a
  `holoRepr`-section, distinct sheets give distinct points, distinct preimages give disjoint
  clusters), which remain as the precise isolated content.
...
* **`localDeg_eq_one_of_regular_fibrePoint`** — at an off-branch fibre point `y` over `coe z`
  (`f.toRiemannSphere y = coe z`) where the chart-pullback derivative of `f.toRiemannSphere` is
  nonzero, the intrinsic local degree is `1` (the regular-value content `hreg_z`). Mirrors the
  genericity `orderAtPoint_sub_eq_one_of_regular_fibrePoint`, reading off `localDeg` instead of the
  shift's order.
* **`localDeg_eq_one_of_regularValueWitness`** — the same, packaged from a `RegularValueWitnessReg`
  whose chosen value is `coe z` (so the `is_regular` certificate supplies the deriv-nonzero), for
  every fibre point. This is the per-value `hreg_z` from the regular-value witness.
* **`exists_regularSlitData`** — at a value `z` off the branch locus (with the regular-value
  witness, finite fibre, and the sphere sheet system + canonical-selection conditions), the
  regular-value tuple `(S, hderiv, hmero, hcoh, hfin_z, hreg_z)` consumed by
  `RealSlitClusterSplitData`.
...
off-branch datum; `hcoh` is the proven `valueChartTrace_eq_sphereSheetFibreTrace` wired for the
canonical selection (the eventual canonical-fibre conditions are the proven
`canonicalFibreSelection_hΦinjReg`/ `hΦrangeReg` + the sheet-system continuity). No custom axiom; no
unproved obligation on a false statement; no false/junk/circular field.

## References

* Forster, *Lectures on Riemann Surfaces* (GTM 81), §4 (the cover is a covering off the branch
  locus).
...
/-- **A regular sphere-fibre point has local degree `1`** (the `hreg_z` content). At a fibre point
`y` over `coe z` (`f.toRiemannSphere y = coe z`) where the chart-pullback derivative of
`f.toRiemannSphere` (read through the affine chart at `coe z`) is nonzero, the intrinsic local
degree is `1`:

> `localDeg f (coe z) y = 1`.

This is the regular-value content: off the branch locus the cover is unramified. The proof mirrors
`orderAtPoint_sub_eq_one_of_regular_fibrePoint`: `y` is a non-pole (its sphere value `coe z` is
finite), the geometric chart pullback `g = holoRepr ∘ chart⁻¹` has `g (chart y) = z` with nonzero
derivative (the deriv transfers from `chartCoe ∘ F ∘ chart⁻¹ =ᶠ g`), so `g − z` has a simple zero
...
/-- **Every fibre point over an off-branch value has local degree `1`** (the `hreg_z` content,
packaged from `branchLocus`). For a nonconstant cover `f` (`f.div ≠ 0`) and a value `z` off the
branch locus (`coe z ∉ branchLocus f.toRiemannSphere`), every preimage `x ∈ F⁻¹(coe z)` is
unramified:

> `localDeg f (coe z) x = 1`.

The regular-value witness at `coe z` (`exists_regularValueWitnessReg_value_eq`, available off the
critical-value set = branch locus) supplies the chart-pullback-deriv-nonzero certificate at each
fibre point, and `localDeg_eq_one_of_regular_fibrePoint` reads off the local degree. -/
theorem localDeg_eq_one_of_notMem_branchLocus (f : MeromorphicFunction X)
  ...
  obtain ⟨w, hw⟩ := Jacobians.Discharge.ContMDiff.Degree.exists_regularValueWitnessReg_value_eq
    ...
    -- `coe z ∉ branchLocus = coe z ∉ criticalValuesGeneral` (defeq:
    -- `branchLocus = f '' criticalSet`, `criticalSet = criticalSetGeneral`,
    -- `criticalValuesGeneral = f '' criticalSetGeneral`).
...
/-- **The regular-fibre primitives `hfin_z`/`hreg_z` at an off-branch value.** For a nonconstant
cover `f` and a value `z` off the branch locus, the fibre over `coe z` is finite and every local
degree there is `1` — exactly the two regular-fibre inputs (`hfin_z`, `hreg_z`) of
`RealSlitClusterSplitData` / `FibreClusterTopology.ofClusterSplitData`. -/
...
/-! ## The sheet-system regularity `hderiv`/`hmero` (from a sphere sheet system off the branch
locus) -/

/-- **`hderiv` for a sphere sheet system off the branch locus.**  For a nonconstant cover `f` and a
value `z` off the branch locus, with `S` a sphere sheet system of `F = f.toRiemannSphere` at
`coe z`, every sheet point `S.sheet k (coe z)` is a regular point of `f`: the chart-pullback
derivative of `f.holoRepr` there is nonzero. Each sheet point has sphere value `coe z`
(`sheet_section` at the base `coe z ∈ S.V`), off the branch locus, so `sheet_holoRepr_deriv_ne_zero`
applies. -/
...
theorem sphereSheet_hmero {X : Type*} [TopologicalSpace X] [ChartedSpace ℂ X] (g : MeromorphicFunction X)
    ...
    ∀ k, MeromorphicAt
      ...
      ((chartAt ℂ (S.sheet k (((z : ℂ) : RiemannSphere))))
        (S.sheet k (((z : ℂ) : RiemannSphere)))) := fun _ => g.meromorphic _
...
theorem sphereSheet_hsheetMem {X : Type*} [TopologicalSpace X] [ChartedSpace ℂ X] [IsManifold 𝓘(ℂ) ω X]
  ...
  have hev : ∀ i : Fin S.n, ∀ᶠ b' in 𝓝 z,
    ...
    have := hcont.preimage_mem_nhds (show
      (chartAt ℂ (S.sheet i (((z : ℂ) : RiemannSphere)))).source ∈ 𝓝 (S.holoReprSheet i z)
    from hsrc)
...
/-- **The regular-value coherence `hcoh` for the canonical full-fibre selection.** At a value `z`
off the branch locus, with a sphere sheet system `S` of `F = f.toRiemannSphere` at `coe z`, the
geometric trace of the canonical selection equals the sphere-sheet fibre trace:

> `valueChartTrace ω₀ f (canonicalFibreSelection g.toFun f hdiv) z
> = (fibreTrace ω₀ f (ofSphereSheetSystem S (sphereSheet_hderiv …) (sphereSheet_hmero
  …))).traceCoeff z`.

This is the proven `valueChartTrace_eq_sphereSheetFibreTrace`, wired for the canonical selection:
the sheet regularity is `sphereSheet_hderiv`/`sphereSheet_hmero`, the canonical-fibre conditions are
`canonicalFibreSelection_hΦinjReg`/`hΦrangeReg` (the `g`-meromorphy is `g.meromorphic`, free), and
the sheet-system conditions are `sphereSheet_hsheetInj`/`sphereSheet_hsheetMem`. -/
theorem canonicalSelection_hcoh (ω₀ : HolomorphicOneForms X) (g : MeromorphicFunction X)
  ...
  -- The `g`-meromorphy near `z` (free from `g.meromorphic`), feeding the canonical-fibre
  -- conditions.
...
atoms above, so `RealSlitClusterSplitData` reduces to **exactly** the sphere sheet system `S` and
the §5 normal-form section facts (`hcs_sec`/`hcs_np`/`hwithin`/`hcross`/`hsrc`/`hsheet_diff`). -/

/-- **`RealSlitClusterSplitData` from the sheet system + the §5 section facts.** At a regular slit
value `z` off the branch locus *on the slit* `Sset` (`hz_slit`), given a sphere sheet system `S` of
`F = f.toRiemannSphere` at `coe z` and the §5 normal-form section facts for the cluster data `Cl`
(whose multiplicity matches the fibre's, `hmult`), build the `RealSlitClusterSplitData`. The
regular-value primitives are discharged: `hderiv` (`sphereSheet_hderiv`), `hmero`
(`sphereSheet_hmero`), `hcoh` (`canonicalSelection_hcoh`), and `hfin_z`/`hreg_z`
(`regularFibre_primitives_of_notMem_branchLocus`). The cluster-sheet differentiability `hsheet_diff`
is also discharged from the `Cl` slit fields (`hw₀_diff`/`hs_an_sheet`, via
`clusterSheet_differentiableAt`). Only the genuine §5 normal-form geometry (the section property
`hcs_sec`/`hcs_np`, the distinctness `hwithin`/`hcross`, and the chart-target locality `hsrc`)
remains as input — the precise isolated content. -/
noncomputable def RealSlitClusterSplitData.ofRegularValue {ω₀ : HolomorphicOneForms X}
  ...
  hsheet_diff := fun i j => by
    -- The cluster sheet `s(ζʲ·w₀ z)` is differentiable: `w₀` differentiable on the slit
    -- (`hw₀_diff`) and `s` analytic at the sheet argument `ζʲ·w₀ z` (`hs_an_sheet`,
    -- `j < (Cl i).m`).
