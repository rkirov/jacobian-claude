...
cluster sum over *all* preimages, at the *genuine* cluster sheet points
`clusterSheet (sec ℓ) (ζ ℓ) (w₀ ℓ) j` (the local inverse `sₗ = ηₗ⁻¹` of the Forster §5 normal form,
**not** the first-order `wp + ζʲ w₀`) — and wires it to a `∑Res = 0` capstone.

It corrects the *two* soundness failures of the single-cluster `RamifiedSheetData.hgeom_slit` (the
#13 bad field): (1) the OUTER sum ranges over the **whole** fibre `F⁻¹(z)` (all preimages
contribute), and (2) the sheet points are the **genuine** `clusterSheet` points.
...
part** of `H` at `0`) collapses it to `ramifiedTraceTerm (principal part of H) c z` plus the
analytic remainder's sheet sum. The residue of `H`'s principal part at `0` is `formFnResidue ω₀ g p`
by the proven residue **change-of-variables** atom `residueChangeOfVariables` (`Res` is
biholomorphism-invariant: `Res₀(h(s u)·s'(u)) = Res_{s 0}(h) = Res_p(α)`).

Summing the per-cluster collapses over **all** preimages (`RamifiedFullFibreClusterGeometry`) gives
the single-valued meromorphic trace germ `T = ∑_ℓ ramifiedTraceTerm_ℓ + ∑_ℓ Rem_ℓ`, whose residue at
`c` is `∑_ℓ formFnResidue ω₀ g pₗ` — the full pole-fibre residue sum. This is exactly a
`RamifiedCenterFacts` (`SerreResidueRamifiedCenter.lean`) over the whole fibre, fed into the
`hoff_cs`-free capstone `residueTheorem_ofRamifiedCenters_genus0_mod`.

## What is delivered

* `clusterSheet_deriv` — the chain-rule derivative
  `(d/dz)[clusterSheet s ζ w₀ j z] = s'(ζʲ w₀ z)·ζʲ·w₀'(z)`.
* `clusterSheetSum_eq_straightenedSheetSum` — the cluster sum equals the first-order sheet sum of
  the straightened integrand `H(u) = h(s u)·s'(u)`.
* `straightenedIntegrand_residue` — `Res₀(H principal part) = formFnResidue ω₀ g p` (change of
  variables).
* `ClusterTraceData` — the per-cluster genuine data (the `clusterSheet` analogue of
  `RamifiedSheetData`, with *no* single-preimage restriction and the genuine straightened
  integrand).
...
Uses the **full fibre** (all preimages — the #13 lesson) and the **genuine** sheet points
`clusterSheet` (via the proven `exists_clusterSplit`/`exists_wpow_normalForm`). The `m`-sheet SUM
(the roots-of-unity atom cancels the chain-rule `1/m`; no single-sheet `m·Res`). Residue invariance
is the proven `residueChangeOfVariables`. No false/junk/circular field; no custom axiom; no unproved
obligation on a false statement.
...
noncomputable def straightenedIntegrand (ω₀ : HolomorphicOneForms X) (g : X → ℂ) (p : X)
    (s : ℂ → ℂ) :
    ℂ → ℂ :=
...
`p`'s chart centre, `s` is analytic at `0` with `s 0 = chartAt ℂ p p`. (Composition of the
meromorphic `chartIntegrand` with the analytic biholomorphism `s`, times the analytic `s'`.) -/
...
(`h(s(ζʲ w₀ z))·s'(ζʲ w₀ z) = H(ζʲ w₀ z)`, and the cluster-sheet derivative `s'(ζʲ w₀ z)·ζʲ·w₀'(z)`
reassociates against the straightened first-order derivative `ζʲ·w₀'(z)`.) -/
...
(`w₀ z ≠ 0`, `w₀(z)ᵐ = z − c`, `w₀'(z) = (1/m) w₀(z)^{1−m}`), the geometric `m`-sheet cluster sum of
the chart integrand `h = chartIntegrand ω₀ g p` along the *genuine* cluster sheets
`clusterSheet s ζ w₀ j` collapses to the single-valued `ramifiedTraceTerm` of the straightened
integrand's Laurent principal part, plus the remainder sheet sum:

> `∑_{j<m} h(clusterSheet s ζ w₀ j z)·(d/dz)[clusterSheet s ζ w₀ j z]`
>   `= ramifiedTraceTerm (Icc 1 N) b (k ↦ −k) m c z + ∑_{j<m} R(ζʲ w₀ z)·(d/dz)[0 + ζʲ w₀ z]`,

where `(N, b, R)` is the Laurent principal-part split of the straightened integrand
`H(u) = h(s u)·s'(u)` at `0` (`H =ᶠ[𝓝[≠] 0] negTail 0 b N + R`), *provided* the split holds at each
sheet argument `ζʲ w₀ z` (`hH_split`). Pure algebra (the proven roots-of-unity atom
`ramifiedSheetSum_laurentPoly`), with the genuine sheet points. -/
...
principal-part split of the straightened integrand `H = h(s ·)·s'(·)`, and the single-valued
analytic remainder trace `Rem` (the symmetric-function descent of the `m`-sheet remainder sum,
supplied as data — exactly the honest decomposition of `RamifiedSheetData`, but at the **genuine**
`clusterSheet` points, with **no** single-preimage restriction).

Summing the per-cluster collapses over the whole fibre (`RamifiedFullFibreClusterGeometry`) builds
the SOUND full-fibre `RamifiedCenterFacts`. -/

/-- **Per-cluster genuine trace data** at one preimage `p` of the value-centre `c` (the
`clusterSheet` analogue of `RamifiedSheetData`, with no single-preimage restriction). Carries the
Forster §5 normal-form local inverse `s = η⁻¹`, the slit-branch `w₀`, the Laurent principal-part
split of the straightened integrand `H = chartIntegrand ω₀ g p (s ·)·s'(·)` at `0`, and the
single-valued analytic remainder trace `Rem`. -/
structure ClusterTraceData (ω₀ : HolomorphicOneForms X) (g : X → ℂ) (p : X) (c : ℂ)
    (S : Set ℂ) where
  ...
  `exists_principalPart_meromorphicAt`): on the slit, the straightened integrand `H` decomposes as
  `negTail 0 + ppR` at each sheet argument. -/
...
/-- The single-valued **per-cluster trace germ**
`Tₗ := ramifiedTraceTerm (principal part of H) + Rem` (a meromorphic function of `z − c` by
construction). -/
...
genuine `clusterSheet` points) equals the single-valued per-cluster trace `Tₗ`. Proof: the
per-cluster collapse `clusterTraceSum_collapse` (the genuine sheet points + the roots-of-unity atom)
gives `ramifiedTraceTerm + remainder sheet sum`, and `hRem_slit` rewrites the remainder sheet sum as
`Rem`. -/
theorem clusterSum_eq_clusterTrace_slit (D : ClusterTraceData ω₀ g p c S) {z : ℂ} (hz : z ∈ S) :
    (∑ j ∈ Finset.range D.m,
        chartIntegrand ω₀ g p (clusterSheet D.s D.ζ D.w₀ j z)
          * deriv (clusterSheet D.s D.ζ D.w₀ j) z)
      = D.clusterTrace z := by
...
(`resAt_straightenedIntegrand`, since the principal part of `H` carries the same residue as `H`);
the analytic `Rem` adds `0`. -/
theorem resAt_clusterTrace (D : ClusterTraceData ω₀ g p c S)
  ...
  rw [hRes]
  -- The residue of `H`'s principal part at `0` = residue of `H` at `0` (the remainder `ppR` is
  -- analytic) = `formFnResidue ω₀ g p` (change of variables).
...
`ClusterTraceData` is inhabited for **every** `m ≥ 1` (the genuine `√(z − c)` slit branch at
`m = 2`), so it is **not** a disguised `False` at a ramification multiplicity. Witness: the zero
numerator `g ≡ 0` (so the chart integrand is `0`), with the trivial straightening
`s u = chart_p p + u` (a genuine local biholomorphism: analytic, `deriv = 1 ≠ 0`) and the `cpow`
slit branch `w₀ z = (z − c)^{1/m}` (which exists for any `m`). Every analytic field is `0 = 0`. -/

/-- **`ClusterTraceData` is inhabitable for every `m ≥ 1`** (non-vacuity / soundness witness).  Zero
numerator `g ≡ 0`, trivial straightening `s u = chart_p p + u`, `cpow` slit branch
`w₀ z = (z − c)^{1/m}` on the slit `S = {z | z − c ∈ slitPlane}`. The branch exists for **any** `m`
(no monodromy contradiction), so the structure is inhabited at genuinely ramified multiplicities —
e.g. `m = 2`. -/
...
/-- **`ClusterTraceData` is inhabited at the ramified multiplicity `m = 2`** — the genuine
`√(z − c)` slit branch (`ζ = −1`). The decisive soundness witness: the structure is satisfiable at a
genuinely ramified multiplicity, not just `m = 1`. -/
...
A `FullFibreClusterData` bundles, at a value-centre `c`, the *whole* pole fibre
`D : FibreRamifiedData` with a per-preimage `ClusterTraceData` (on a common slit), the single-valued
meromorphy `hvct_mero` of the geometric trace, and the **full-fibre cluster geometric identity** on
the slit (the geometric trace is the full-fibre sum of all per-cluster sheet sums —
`RamifiedFullFibreClusterGeometry` on the slit).

From it we build the SOUND full-fibre `RamifiedCenterFacts` whose germ `T = ∑ₗ clusterTraceₗ` is
single-valued meromorphic, equals `valueChartTrace` on a punctured neighbourhood (identity theorem),
and has residue `∑ₗ formFnResidue ω₀ g pₗ` (the full pole-fibre sum). This is the corrected analogue
of `RamifiedCenterFacts.ofSheetData` (which used the false single-cluster `hgeom_slit`). -/

/-- **The full-fibre cluster data at a value-centre `c`** (the SOUND geometric input — full fibre,
genuine cluster sheets). Bundles the whole pole fibre `D`, a per-preimage `ClusterTraceData` `Cl i`
on a common slit `S`, the geometric trace's single-valued meromorphy `hvct_mero`, and the full-fibre
...
structure FullFibreClusterData (ω₀ : HolomorphicOneForms X) (g : X → ℂ) (f : MeromorphicFunction X)
  ...
  /-- **The full-fibre cluster geometric identity on the slit** (the SOUND `hgeom`, full fibre +
  genuine cluster sheets): the geometric trace equals the sum over *all* preimages of the
  per-cluster sheet sums at the genuine `clusterSheet` points. -/
...
/-- **The full-fibre single-valued trace germ** `T := ∑ₗ clusterTraceₗ` (a finite sum of
single-valued meromorphic per-cluster traces). -/
noncomputable def fibreTrace (F : FullFibreClusterData ω₀ g f Φ c) : ℂ → ℂ :=
  fun z => ∑ i, (F.Cl i).clusterTrace z

/-- **The full-fibre slit identity.** On the slit, the geometric trace equals
`T = ∑ₗ clusterTraceₗ`. Proof: the full-fibre cluster geometric identity `hgeom_fibre` writes
`valueChartTrace` as the sum over all preimages of the per-cluster sheet sums; each per-cluster
sheet sum collapses (the per-cluster slit identity `clusterSum_eq_clusterTrace_slit`, the genuine
sheet points + the roots-of-unity atom) to `clusterTraceₗ`. -/
...
/-- **`hcoh` (DERIVED via the identity theorem):** `valueChartTrace =ᶠ[𝓝[≠] c] T`. Both meromorphic
at `c` (`hvct_mero` / `meromorphicAt_fibreTrace`) and agree on the slit (`eqOn_fibreTrace_slit`),
which accumulates at `c` (`hS_acc`). -/
theorem hcoh (F : FullFibreClusterData ω₀ g f Φ c) :
  ...
  eventuallyEq_of_meromorphic_eqOn_slit F.hvct_mero F.meromorphicAt_fibreTrace F.hS_acc
    (fun _ hz => F.eqOn_fibreTrace_slit hz)

/-- **The full-fibre residue.**  `resAt T c = ∑ᵢ formFnResidue ω₀ g (D.xs i)` (the full pole-fibre
residue sum). Residue of the finite sum splits termwise (`resAt_finsum`); each term is the
per-cluster residue `formFnResidue ω₀ g (D.xs i)` (`resAt_clusterTrace` via change-of-variables). -/
...
/-- **The SOUND full-fibre `RamifiedCenterFacts`** (the corrected analogue of `ofSheetData`). From
the full-fibre cluster data `F` and the pole-fibre enumeration conditions (`hD_mem`/`hD_surj`),
build the `RamifiedCenterFacts` whose germ `T = ∑ₗ clusterTraceₗ` is single-valued meromorphic,
equals `valueChartTrace` on a punctured neighbourhood (`hcoh`, derived via the slit + identity
theorem), and has residue `∑ᵢ formFnResidue ω₀ g (D.xs i)` (`resAt_fibreTrace`, via
change-of-variables). **Full fibre** (all preimages — the #13 fix) at the **genuine** `clusterSheet`
points; no false/circular field. -/
...
SOUND full-fibre `RamifiedCenterFacts` (`toRamifiedCenterFacts`) as the `Nonempty`-wrapped
per-centre obligation `ExistsRamifiedCenterFacts` that the `hoff_cs`-free capstone consumes. This is
the corrected analogue of `existsRamifiedCenterFacts_ofSheetData` (which used the false
single-cluster `hgeom_slit`). -/
theorem existsRamifiedCenterFacts_ofFullFibre (F : FullFibreClusterData ω₀ g f Φ c)
    {poles : Finset X}
    (hD_mem : ∀ i, F.D.xs i ∈ poles)
    (hD_surj : ∀ a ∈ poles, f.toRiemannSphere a = ((c : ℂ) : RiemannSphere) → ∃ i, F.D.xs i = a) :
...
ramified preimages), the zero numerator `g ≡ 0` inhabits a `FullFibreClusterData`: each preimage
gets a `clusterTraceData_slit` cluster (the genuine `cpow` slit branch), `valueChartTrace ≡ 0`
(empty selection), and the full-fibre geometric identity is `0 = ∑ᵢ ∑ⱼ 0 = 0`. The explicit
two-preimage `m = 2` instance `fullFibreClusterData_twoSheet` then witnesses a *genuine
multi-preimage ramified* case, **not** a disguised `False` and **not** single-preimage. -/

/-- **`FullFibreClusterData` from any injective fibre enumeration** (the multi-preimage
non-vacuity). For the zero numerator `g ≡ 0` and the empty selection, *any* `FibreRamifiedData D`
over `c` (injective, multiplicities `≥ 1`) inhabits a `FullFibreClusterData` — per-preimage
`clusterTraceData_slit` clusters (genuine `cpow` slit branches), `valueChartTrace ≡ 0`, and the
full-fibre identity `0 = ∑ᵢ ∑ⱼ 0`. This shows the structure imposes **no single-preimage
restriction** (the #13 fix) and is constructible at genuinely ramified multiplicities. -/
...
centre `cs i` of `α = ω₀·g`, the canonical full-fibre selection carries a `FullFibreClusterData`
(the genuine full-fibre Forster §5 cluster geometry — *all* preimages, genuine `clusterSheet`
points) whose preimages enumerate exactly the poles of `α` in the fibre `F⁻¹(coe (cs i))`. This is
the SOUND replacement of `RealCoverRamifiedCenters` (which packaged the single-cluster
`RamifiedSheetData`). -/
...
This routes through the `hoff_cs`-free capstone `residueTheorem_ofRamifiedCenters_genus0_mod`, fed
the SOUND full-fibre `RamifiedCenterFacts` (`existsRamifiedCenterFacts_ofFullFibre`) at each centre
— the finite centres may be **ramified**, and the geometry is the genuine full-fibre cluster
identity (NOT the unsound single-cluster `hgeom_slit`). -/
...
finite pole-value centres (`m = 0`), the per-centre data is vacuously inhabited —
`FullFibreRamifiedCenters` is **not** a disguised `False`. -/
...
(`SerreResidueRamifiedClusterSplit.lean`) is exactly the geometric content of a
`FullFibreClusterData`, read on the slit with a `Fin r` enumeration of the fibre. We prove it holds
(after reindexing the preimage type `D.ι` to `Fin (card D.ι)` via `Fintype.equivFin`), discharging
the named target. -/

/-- **`RamifiedFullFibreClusterGeometry` is proven from a `FullFibreClusterData`.** The morning's
named target — the full-fibre geometric identity at the genuine `clusterSheet` points — holds for
the `Fin r` enumeration `pre := D.xs ∘ e.symm` of the fibre (`e : Fin r ≃ D.ι`, `r := card D.ι`),
with the per-preimage cluster data `(mult, ζ, w₀, sec)` read off `F.Cl`. This is the geometric
`hgeom_fibre` field reindexed through the equiv. **Sound:** the full fibre (all preimages) at the
genuine `clusterSheet` points — the two corrections over the unsound single-cluster `hgeom_slit`. -/
