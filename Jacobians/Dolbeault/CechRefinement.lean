/-
  The Čech **refinement map** between finite covers.

  The finiteness statement `CechFinitenessWiring.exists_cechModel` must, for an arbitrary Leray
  cover `𝔘`, produce a sup-norm Montel model `c` with `𝔘.cechH1 D ≃ₗ[ℂ] c.supH1`, while the model
  geometry (`CechModelGeometry.chartCoverOverlapData`) and the Čech↔Dolbeault comparison
  (`DolbeaultComparisonEquiv.comparison_linearEquiv`) are built from a specific chart-disk cover.
  Bridging the two requires **`cechH1` to be independent of the (Leray) cover** — a chain of
  refinement isomorphisms `cechH1 𝔘 D ≅ cechH1 (common refinement) D ≅ cechH1 chartCover D`.

  Mathlib has no Čech-cohomology refinement / nerve / Leray theorem, so the bridge is built by
  hand.  This file contains the part that is pure restriction algebra:

    * `IsRefinement 𝔙 𝔘 r` — `r : 𝔙.ι → 𝔘.ι` is a refinement-index map (`𝔙.U j ≤ 𝔘.U (r j)`);
    * the induced **reindex+restrict** cochain maps `refineC0/refineC1/refineC2 :
      Cochain^k 𝔘 → 𝔙` (germ restriction `rawRestrictG` along `𝔙.U j ≤ 𝔘.U (r j)`, reindexed
      by `r`);
  ...
  The cover-independence itself (`cechH1 𝔘 ≅ cechH1 𝔙` for Leray covers) is exactly the statement
  that `refineH1` is a bijection; it is proven downstream: homotopy/index-independence of the
  refinement map in `CechRefinementHomotopy`, unconditional injectivity (Forster 12.4) in
  `CechRefinementInjective`, and the Leray isomorphism in `CechRefinementLeray`.
-/
...
This is the genuinely reusable algebraic content (the eventual cover-independence proof needs that
the refinement maps are chain maps, so that they descend to cohomology). Both directions collapse
nested germ restrictions via `rawRestrictG_comp_apply` — `δ⁰` of a restriction equals the
restriction of `δ⁰` on the smaller overlap. -/
...
The cocycle-level reindex+restrict map (`refineC1` restricted to cocycles) carries coboundaries
to coboundaries, so it descends to the `H¹ = Z¹/B¹` quotients (`Submodule.mapQ`).  This is the
comparison arrow `cechH1 𝔘 → cechH1 𝔙` that the cover-independence proof
(`CechRefinementLeray`) shows is an isomorphism for two Leray covers. -/
...
noncomputable def refineH1 [T2Space X] [CompactSpace X] [ConnectedSpace X] [ChartedSpace ℂ X]
    [IsManifold 𝓘(ℂ) ω X] : 𝔘.cechH1 D →ₗ[ℂ] 𝔙.cechH1 D := by
