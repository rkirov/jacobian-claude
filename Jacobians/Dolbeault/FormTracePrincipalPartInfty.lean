...
/-!
# Principal-part extraction across `∞` for the global trace (Miranda §VIII.3 step 3, `∞` part)

`Jacobians.Dolbeault.FormTracePrincipalPart` extracts the finite principal parts of the global trace
coefficient `T : ℂ → ℂ`. Miranda §VIII.3 step 3 also subtracts the **polynomial part at `∞`** to
leave a *global* holomorphic form on the compact `ℂℙ¹`, vanishing by genus-`0` Liouville
...
last fact is the precise, honest shape of `GlobalTrace.hrecip`: the `hrecip` continuity is
*equivalent* to the analytic continuation of `recipCoeff (T − L.R)` vanishing at `0`, i.e. the
holomorphic remainder having no `dζ`-term at `∞`. Because a holomorphic `1`-form on `ℂℙ¹` is `0`
(genus `0`), this is genuine §VIII.3 content — not mechanically dischargeable from meromorphy alone
— so `hrecip` correctly remains part of the irreducible global-trace obligation (its analogue
`hentire` is dischargeable once `T` is junk-free, via `exists_entire_of_finitePoles`).

## Main results

* `recipCoeff_sub` — `recipCoeff` is additive over subtraction
  (`recipCoeff (R₁ − R₂) = recipCoeff R₁ − recipCoeff R₂`).
* `recipCoeff_zero_eval` — `recipCoeff h 0 = 0` for every `h` (the junk `0^{-2}` Jacobian), so the
  `hrecip` continuity is to value `0`. * `continuousAt_recipRemainder_of_vanishing` — given a
  reciprocal-chart principal part `negTail 0 b N` with analytic continuation `R` of the remainder,
  if `R 0 = 0` (the holomorphic remainder vanishes at `∞`) then `recipCoeff T − negTail 0 b N` is
  continuous at `0` — exactly `GlobalTrace.hrecip` once `recipCoeff L.R` germ-matches
  `negTail 0 b N`. This isolates `R 0 = 0` as the irreducible `∞` obligation.
...
`GlobalTrace.hrecip` condition `ContinuousAt (recipCoeff (T − L.R)) 0` is precisely
"`recipCoeff (T − L.R)` *tends to `0`* at `0`", i.e. the analytic continuation of the reciprocal
coefficient of the holomorphic remainder *vanishes at `∞`* — the genus-`0` holomorphic-remainder
content (a holomorphic `1`-form on `ℂℙ¹` has no `dζ`-term at `∞` only if it is `0`). This is the
irreducible §VIII.3 input that `FormTraceLiouville` consumes; it is *not* mechanically dischargeable
from meromorphy alone. -/
...
`exists_principalPart_meromorphicAt`) and analytic continuation `R` of the remainder. If that
analytic continuation *vanishes at `0`* (`hR0 : R 0 = 0` — the holomorphic remainder has no
`dζ`-term at `∞`, the genus-`0` content), then `recipCoeff T − negTail 0 b N` is **continuous at
`0`** (continuity to the junk value `0`). This is the precise, honest shape of `GlobalTrace.hrecip`:
it isolates `R 0 = 0` as the single irreducible §VIII.3 obligation at `∞`. -/
theorem continuousAt_recipRemainder_of_vanishing {T : ℂ → ℂ} {N : ℕ} {b : ℕ → ℂ} {R : ℂ → ℂ}
  ...
  -- Off `0`, the remainder equals `R`; at `0` both equal `0` (junk value of `recipCoeff`,
  -- `R 0 = 0`). Value at `0`: `recipCoeff T 0 − negTail 0 b N 0 = 0 − 0 = 0 = R 0`.
  ...
  rw [ContinuousAt]
  show Tendsto (fun ζ => recipCoeff T ζ - negTail 0 b N ζ) (𝓝 0)
    (𝓝 (recipCoeff T 0 - negTail 0 b N 0))
