...
/-!
# Genus-`0` discharge of the residue-theorem assembly's residual #5 (the `∞`-vanishing of the trace
remainder)

`Jacobians.Dolbeault.SerreResidueTheorem.residueTheorem_of_directGeometry`
(`SerreResidueDirect.lean`) closes the residue theorem `∑Res = 0` from the residue-level
§VIII.3 geometry, but takes as caller hypotheses the **genus-`0` `∞`-vanishing** field-group
`R₀`/`hR₀_an`/`hR₀0`/`hR₀_eq` (the analytic continuation of `recipCoeff (T − L.R)` off `0`,
**vanishing at `0`**) — the "residual #5" that, before this file, was discharged *only* for the
empty-pole case.
...
`continuousAt_recipCoeff_of_vanishing` — equivalent to `ContinuousAt (recipCoeff (T − L.R)) 0`, i.e.
the remainder `T − L.R` being holomorphic at `∞` *with no `dζ`-term* (`T − L.R = o(z⁻²)` at `∞`).
Since `recipCoeff L.R` has a **simple pole** at `0` whose residue is `resAtInfty L.R`
(`resAtInfty_eq_resAt_recipCoeff`) and `recipCoeff T` has a simple pole at `0` whose residue is
`∑_{F a = ∞} formFnResidue` (the `∞`-fibre Lemma 3.2, via `hcoh_full`), continuity of
`recipCoeff (T − L.R)` at `0` **forces those two simple-pole residues to cancel** — which is exactly
the `∞`-residue identity `infty_eq`, and `infty_eq` together with the finite Lemma 3.2 and the `ℂℙ¹`
residue theorem **is** the residue theorem `∑Res = 0`. So discharging `R₀ 0 = 0` for a
*nonempty* trace through the `T = L.R` route is **circular** with the theorem; the repo records the
same fact at `FormTraceGlobalConstruct.lean` (the "`infty_eq` circularity").
...
infinity of an **entire** `1`-form coefficient vanishes, because `recipCoeff h = d/dζ [H(ζ⁻¹)]` for
a global primitive `H` of `h` (`Differentiable.isExactOn_univ`), so its small-circle integral
vanishes (`circleIntegral.integral_eq_zero_of_hasDerivWithinAt`). The entire-ness of `T − L.R` is
the **finite** junk-freeness `hcont_int` (genuinely about the finite centres, *not* `∞`), already an
input.

So this file **fully discharges** the residual-#5 field-group `R₀`/`hR₀_an`/`hR₀0`/`hR₀_eq`,
re-deriving the `∞`-residue identity `infty_eq` from the *proven*
`resAt (recipCoeff (T − L.R)) 0 = 0` (Cauchy) plus the genuine `∞`-coherence `hcoh_full` (#6). It
also re-proves the **finite** Lemma-3.2 residue identity directly from the principal-part extraction
(the residue of `T − L.R` at each finite centre is `0` because it germ-equals an analytic function
there — `exists_laurentForm_principalPart`), *without* the global `T = L.R`.

## What this file proves

* `resAt_recipCoeff_eq_zero_of_entire` — **Cauchy at infinity**: `resAt (recipCoeff h) 0 = 0` for
  `h` entire (`AnalyticOnNhd ℂ h Set.univ`). The non-circular replacement of `R₀ 0 = 0`.
* `holoPunctured_recipCoeff_entire` / `meromorphicAt_recipCoeff_laurent` — the
  `HoloPunctured`/meromorphy inputs of `resAt` additivity at `0`.
* `resAt_eq_laurentR_of_principalPart` — the **finite residue match**
  `resAt T (cs i) = resAt L.R (cs i)` (free from the principal-part extraction; no `R₀`, no global
  `T = L.R`).
* `infty_eq_of_remainderResZero` — the **`∞`-residue identity** from
  `resAt (recipCoeff (T − L.R)) 0 = 0` + `hcoh_full`.
* `globalTraceData_of_genus0` / `residueTheorem_of_directGeometry_genus0` — the residue-level close
  of the residue-theorem assembly **without** the residual-#5 field-group, keeping only the finite
  junk-freeness `hcont_int` and the `∞`-coherence `hcoh_full`.
* `directTraceGeometry_ofCanonicalSimpleInfty_genus0` — the capstone constructor: a
  `DirectTraceGeometry` whose `R₀`/`hR₀_an`/`hR₀0`/`hR₀_eq` fields are discharged internally.
...
reciprocal chart as `resAt (recipCoeff h) 0`) is `0`. This is *Cauchy's theorem at `∞`*:
`recipCoeff h (ζ) = −h(ζ⁻¹)·ζ⁻² = d/dζ [H(ζ⁻¹)]` for a global primitive `H` of `h`, so on any small
circle `C(0, r)` the integral of `recipCoeff h` is the integral of a derivative around a closed loop
avoiding `0`, hence `0` (`circleIntegral.integral_eq_zero_of_hasDerivWithinAt`). This is the
**non-circular** replacement of the genus-`0` `∞`-vanishing `R₀ 0 = 0`: it uses *only* entire-ness,
never `∑Res = 0`. -/
...
`∀ z, HasDerivAt H (h z) z`). On the circle `C(0, r)` (`0 < r < 1`),
`recipCoeff h (ζ) = h(ζ⁻¹)·(−(ζ²)⁻¹)` is the derivative of `ζ ↦ H(ζ⁻¹)` (chain rule, `ζ ≠ 0` on the
circle), so the contour integral vanishes (`circleIntegral.integral_eq_zero_of_hasDerivWithinAt`);
...
`recipCoeff h (ζ) = −h(ζ⁻¹)·ζ⁻²` is holomorphic on `ball 0 1 \ {0}` (the only singularity is the
`ζ⁻²` Jacobian at `0`); the `HoloPunctured` input of `resAt_add` at `0`. -/
...
theorem meromorphicAt_recipCoeff_laurent (L : LaurentForm) : MeromorphicAt (recipCoeff L.R) 0 := by
  ...
  exact ((((MeromorphicAt.const (L.c i) 0).mul hbase)).neg).mul
    ((analyticAt_id.meromorphicAt).zpow _)
...
germ-equals an analytic function `R` off `c` (`hR_eq` — the pole removed, as
`exists_laurentForm_principalPart` supplies), then `resAt T c = resAt L.R c`. *Proof.*
`resAt (T − L.R) c = resAt R c = 0` (analytic), and `resAt` is additive, so
`resAt T c = resAt L.R c`. -/
...
No step assumes `∑Res = 0`: the genus-`0` content is `hcoh_full` (the geometric `∞`-coherence) and
the entire-ness of the remainder; the residue-zero step is Cauchy. -/

/-- **The `∞`-residue identity from the entire remainder + the `∞`-coherence (non-circular).**
Given: `hentire` (the remainder `T − L.R` is entire — the finite junk-freeness), `hcoh_full` (the
`∞`-coherence against the full `∞`-fibre `Dinf_full`), the full `∞`-fibre enumeration data, and the
pole-only `∞`-enumeration with the non-`α`-pole points contributing residue `0`, then
...
theorem infty_eq_of_remainderResZero
  ...
  -- `resAtInfty L.R = resAt (recipCoeff L.R) 0`, then `= resAt (recipCoeff T) 0` via the entire
  -- remainder.
...
/-! ## The residue-level close of the residue-theorem assembly, remaining obligations discharged

`globalTraceData_of_genus0` assembles a `GlobalTraceData` — and hence `∑Res = 0` — from the
residue-level §VIII.3 geometry **without** the residual-#5 field-group
`R₀`/`hR₀_an`/`hR₀0`/`hR₀_eq`. The `∞`-residue identity is re-derived
(`infty_eq_of_remainderResZero`) from the *proven* entire-remainder `∞`-residue
(`resAt_recipCoeff_eq_zero_of_entire`, Cauchy at `∞`) plus the `∞`-coherence `hcoh_full` (#6); the
finite Lemma-3.2 residue identity is `resAt_eq_laurentR_of_principalPart` (free). The **finite**
...
noncomputable def globalTraceData_of_genus0 [Nonempty X]
  ...
  refine
    { L := L
      ...
      infty_eq := ?_ }
  · -- Finite Lemma 3.2:
    -- `∑ resAt(fibreTrace coeff) = ∑ formFnResidue = resAt T (cs i) = resAt L.R (cs i)`.
    intro p hp
    rw [hLcenters] at hp
    simp only [Finset.mem_image, Finset.mem_univ, true_and] at hp
    obtain ⟨i, rfl⟩ := hp
...
/-- **The residue theorem `∑Res = 0` from the residue-level geometry, remaining obligation group
discharged.** The total residue of `α = ω₀·g` over its poles vanishes, from the residue-level
§VIII.3 geometry with the `R₀`/`hR₀_an`/`hR₀0`/`hR₀_eq` field-group **dropped** — the genus-`0`
`∞`-vanishing is re-derived internally (Cauchy at `∞` for the entire remainder + the `∞`-coherence
`hcoh_full`). Only the finite junk-freeness `hcont_int` and the `∞`-coherence `hcoh_full` remain. -/
theorem residueTheorem_of_directGeometry_genus0 [Nonempty X]
    (Φ : (b : ℂ) → FibreRegularData g f b)
    (m : ℕ) (cs : Fin m → ℂ) (ρ : ℝ) (hcs_ball : ∀ i, cs i ∈ ball (0 : ℂ) ρ)
    (hcs_inj : Function.Injective cs) (br : Finset ℂ)
    (hreg : ∀ w ∉ Finset.univ.image cs ∪ br, AnalyticAt ℂ (valueChartTrace ω₀ f Φ) w)
...
`residueTheorem_of_directGeometry_genus0` is satisfiable, not a disguised `False`: for the empty
pole set the empty fibre selection (`Φ` empty, `m = 0`, `br = ∅`, the zero trace `T ≡ 0`) satisfies
every input — including the **finite** junk-freeness `hcont_int` (the zero remainder is continuous)
and the `∞`-coherence `hcoh_full` (both sides `0`). Confirms the residual-#5-discharged inputs are
honest. -/

/-- **Non-vacuity of the residual-#5-discharged residue-level close.**  For the empty pole set,
`residueTheorem_of_directGeometry_genus0` is satisfiable via the empty selection and `br = ∅`,
yielding `∑_{a ∈ ∅} formFnResidue ω₀ g a = 0`. Confirms the inputs are not a disguised `False`. -/
theorem residueTheorem_of_directGeometry_genus0_holomorphic [Nonempty X]
    (ω₀ : HolomorphicOneForms X) (g : X → ℂ) (f : MeromorphicFunction X) :
