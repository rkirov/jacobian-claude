...
* `hgeom_fibre` (the full-fibre cluster identity) + the conservation-of-number count
  ...
  `deg f` preimages — pole *and* non-pole), because the geometric trace is a sum over **all**
  sheets;
...
So the full-fibre residue sum collapses to the pole-only fibre sum (`residueSum_full_eq_poleOnly`,
the proven shared full→pole-only engine, already used in the unramified `Cfull` route), which is the
...
This is exactly the `Cfull` route's residue-level discharge (`hres_fin_of_fullFibreCoherence`), but
with the per-centre trace coherence coming from the **ramified** full-fibre cluster geometry
(`FullFibreClusterData`) instead of the unramified `MovingCoherenceDatum`.

## What is delivered

* `FullFibreClusterData.resAt_patched_filter_of_nonpole` — fact (B, filtered) directly from a
  whole-fibre `FullFibreClusterData` + the non-pole-residue-`0` datum + the pole-fibre surjectivity:
  the residue of the patched trace at `c` equals the fibre-restricted pole-set sum.
...
* `residueSum_eq_zero_of_fullFibreReindex` — `∑Res = 0` from per-centre `FullFibreCenterReindex` +
  `GateAInftyData`.
* `residueSum_eq_zero_of_fullFibreReindex_adaptedFRamified` — the same from an `AdaptedFRamified`
  datum (the `hnonpole` datum then comes for free from `A.hg_an_offpoles`).

## ⚠ Soundness

`D` = the WHOLE fibre (all `deg f` preimages), **never** the poles. Non-pole clusters genuinely
contribute residue `0` (`formFnResidue_eq_zero_of_analyticAt`, `α` holomorphic). The reconciliation
is the proven `residueSum_full_eq_poleOnly`. No all-poles assumption; no full Riemann–Roch (no
circularity); no custom axiom; no unproved obligation on a false statement; no false/junk/circular
field. Sanity: at a fibre with both poles and non-poles the non-pole contribution is `0` and the
formula holds (the explicit non-vacuity witnesses `fullFibreCenterReindex_zero` / `…_empty`).
...
`FullFibreClusterData` already carries the SOUND whole-fibre geometry: its `D` is the whole fibre,
and `resAt_fibreTrace` reads `resAt T c = ∑ᵢ formFnResidue ω₀ g (D.xs i)` over *all* preimages.
Combined with the geometric coherence `hcoh` (`valueChartTrace =ᶠ[𝓝[≠] c] fibreTrace`) the *patched*
trace germ-equals `fibreTrace`, so its residue at `c` is the same whole-fibre sum. Splitting that
sum by pole-membership — non-poles → `0` — gives the pole-fibre residue sum, exactly fact (B,
filtered). -/
...
`valueChartTrace` (patch inert off `c`), which germ-equals `fibreTrace` (the geometric coherence
`hcoh`), which is meromorphic (`meromorphicAt_fibreTrace`). -/
...
/-- **The patched residue is the whole-fibre residue sum.**
`resAt (patched) c = ∑ᵢ formFnResidue ω₀ g (D.xs i)` over the **whole fibre**: take residues across
the germ-equality (`resAt_congr`), then `resAt_fibreTrace`. -/
...
/-- **Fact (B, filtered), the SOUND reconciliation.** From a **whole-fibre** `FullFibreClusterData`
`F`, the non-pole-residue-`0` datum (`hnonpole`: a preimage off `poles` has `formFnResidue = 0`,
true by `α` holomorphy), and the pole surjectivity (`hD_surj`: every α-pole of the fibre is some
`D.xs i`), the residue of the patched trace at `c` equals the **fibre-restricted pole-set sum**:

> `resAt (valueChartTracePatched ω₀ f Φ br) c
>    = ∑_{a ∈ poles, F a = coe c} formFnResidue ω₀ g a`.

Proof: `resAt_patched` reads the residue as the **whole-fibre** sum
`∑ᵢ formFnResidue ω₀ g (D.xs i)`; the proven full→pole-only engine `residueSum_full_eq_poleOnly`
collapses it to the sum over the pole sub-fibre `{i // D.xs i ∈ poles}` (non-poles contribute `0`),
which the `Finset` re-indexing identifies with the fibre-restricted pole-set sum (via
`hD_inj`/`hD_surj`). **No `hD_mem` (no all-poles assumption)** — the whole fibre is enumerated, and
the non-poles drop out at the residue level. -/
theorem resAt_patched_filter_of_nonpole (F : FullFibreClusterData ω₀ g f Φ c) (br : Finset ℂ)
  ...
  rw [residueSum_full_eq_poleOnly (poles := poles) F.D.xs xsPo F.hD_inj hpo_inj hpole_image
    hnonpole]
...
The corrected replacement of `FibreClusterReindex`. At a finite pole-value centre `c`, it carries
the **whole-fibre** `FullFibreClusterData` (the genuine §5 cluster geometry — all preimages, genuine
`clusterSheet` points), the eventual analyticity / pole-order bound feeding the meromorphy of the
geometric trace (already a field of `FullFibreClusterData` as `hvct_mero`, so not duplicated here),
the non-pole-residue-`0` datum, and the pole surjectivity. **There is no `hD_mem` (no all-poles
...
`FibreClusterReindex`: a **whole-fibre** `FullFibreClusterData` for the canonical selection, plus
the residue-level reconciliation data:
...
**No `hD_mem` (no all-poles assumption):** the fibre `F.D` is the WHOLE fibre, and the non-pole
points genuinely drop out at the residue level. This is the fix of the full-fibre-vs-pole
inconsistency. -/
structure FullFibreCenterReindex (ω₀ : HolomorphicOneForms X) (g : X → ℂ)
    (f : MeromorphicFunction X)
    (hdiv : (f.div : Divisor X) ≠ 0) (poles : Finset X) (c : ℂ) where
  /-- The whole-fibre cluster data for the canonical selection (`F.D` = the whole fibre
  `F⁻¹(coe c)`). -/
  F : FullFibreClusterData ω₀ g f (canonicalFibreSelection g f hdiv) c
  /-- A non-pole preimage contributes residue `0` (`α` holomorphic;
...
/-- **Fact (B, filtered) from a `FullFibreCenterReindex`** — the residue of the patched trace at `c`
is the fibre-restricted pole-set sum (the SOUND reconciliation, non-poles → `0`). -/
...
Feeding the per-centre facts (A)/(B, filtered) — now SOUNDLY built from the whole-fibre cluster
geometry — into the proven `hoff_cs`-free `residueSum_eq_zero_of_centerFacts` closes the
residue-theorem assembly. -/

/-- **The residue theorem `∑Res = 0` from per-centre `FullFibreCenterReindex` (the SOUND
full-fibre close).** Given the per-centre SOUND full-fibre residual `FullFibreCenterReindex` (whole
fibre, non-poles → residue `0` — NO all-poles assumption) and the already-residue-theorem-discharged
...
theorem residueSum_eq_zero_of_fullFibreReindex {ω₀ : HolomorphicOneForms X} {g : X → ℂ}
    ...
    ∑ a ∈ poles, formFnResidue ω₀ g a = 0 :=
  residueSum_eq_zero_of_centerFacts (canonicalFibreSelection g f hdiv) m cs A.ρ A.hcs_ball
    A.hcs_inj br
...
/-- **The residue theorem `∑Res = 0` from an `AdaptedFRamified` + per-centre
`FullFibreCenterReindex`.** For a genuine meromorphic numerator `g`, an `AdaptedFRamified` datum `A`
(the off-centre/∞ genericity, **admitting ramified finite pole fibres**) and at each finite
pole-value centre `A.cs i` a SOUND full-fibre residual `FullFibreCenterReindex`, the total residue
of `α = ω₀·g` vanishes:
...
multi-preimage ramified fibre — and crucially it imposes **no** restriction that the fibre be
all-poles (the inconsistency we fixed). For the zero numerator `g ≡ 0` the canonical-selection
geometric trace vanishes identically, so for *any* injective whole fibre `D` over `c` (any
preimages, any multiplicities `≥ 1`, **any mix of pole and non-pole points**) we inhabit a
`FullFibreCenterReindex` with the empty pole set: `hnonpole` is then vacuously the residue-`0` fact
(and indeed `formFnResidue ω₀ 0 a = 0` for the zero numerator), and `hD_surj` is vacuous. With
`poles = ∅` no preimage is a pole, so the fibre is *entirely non-pole* — the precise generic case
that broke `FibreClusterReindex.hD_mem`. -/
...
/-- **The whole-fibre `FullFibreClusterData` for the canonical selection at the zero numerator.**
For `g ≡ 0` the canonical-selection geometric trace vanishes (`valueChartTrace_zero_numerator`), so
*any* injective whole fibre `D` over `c` carries a `FullFibreClusterData` for the *canonical*
selection — each preimage a genuine `clusterTraceData_slit` cluster (the `cpow` slit branch),
`hgeom_fibre` the identity `0 = ∑ᵢ ∑ⱼ 0`. This is the canonical-selection analogue of
`fullFibreClusterData_zero` (which uses the empty selection), built to match the
`canonicalFibreSelection` parameter of `FullFibreCenterReindex`. -/
...
/-- **`FullFibreCenterReindex` is inhabited at a genuine all-non-pole ramified fibre** (the #17
soundness witness — the precise generic case `FibreClusterReindex` could not express). For the zero
numerator `g ≡ 0` and *any* injective whole fibre `D` over `c` (any preimages, any multiplicities
`≥ 1`), with the **empty** pole set, a `FullFibreCenterReindex` is inhabited: the whole-fibre
`FullFibreClusterData` is `fullFibreClusterData_zero_canonical`, `hnonpole` holds (zero-numerator
residue `0`), and `hD_surj` is vacuous (empty poles). Every preimage is a non-pole, yet the
structure is satisfiable — confirming **no all-poles assumption** and that it is **not** a disguised
`False`. -/
noncomputable def fullFibreCenterReindex_zero (ω₀ : HolomorphicOneForms X)
    (f : MeromorphicFunction X)
...
/-- **`FullFibreCenterReindex` is inhabited at a genuine MIXED fibre** (the decisive #17 sanity
check — both poles and non-poles in the same fibre). For the zero numerator `g ≡ 0` and any
injective whole fibre `D` over `c`, with **any** pole set `poles` whose fibre part lies in the fibre
(`hpoles_sub`: `poles ⊆ range D.xs`, so some fibre points are poles and others may not be), a
`FullFibreCenterReindex` is inhabited. This is the case the old `FibreClusterReindex.hD_mem` (which
demanded the *whole* fibre be poles) could not express: here a *proper* nonempty subset of the fibre
can be the poles, the rest are non-poles contributing residue `0`. `hnonpole` holds (zero-numerator
residue `0` everywhere), and `hD_surj` holds since `poles ⊆ range D.xs`. -/
...
numerator, at a fibre over `c` enumerated by an injective `D` (genuinely mixing pole/non-pole points
via `poles ⊆ range D.xs`), the residue of the patched trace at `c` equals the fibre-restricted
pole-set sum, which is `0` — the non-pole contribution is `0` (zero numerator) and the pole
contribution is `0` too, confirming the reconciliation
`resAt = ∑_{a ∈ poles, F a = coe c} formFnResidue` is correct at a MIXED fibre (not secretly
all-poles or all-non-poles). -/
theorem resAt_patched_filter_mixed_zero (ω₀ : HolomorphicOneForms X) (f : MeromorphicFunction X)
  ...
  exact (fullFibreCenterReindex_zero_mixed ω₀ f hdiv c D hD_inj poles
    hpoles_sub).resAt_patched_filter br

/-- **Non-vacuity of `residueSum_eq_zero_of_fullFibreReindex`'s premise (no finite pole-value
centres).** When there are no finite pole-value centres (`m = 0`), the per-centre residual is
vacuously inhabited — the SOUND full-fibre route is **not** a disguised `False`. -/
