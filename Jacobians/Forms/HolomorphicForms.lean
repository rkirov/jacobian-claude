...
-- defined in `Jacobians.Forms.Genus` (to allow `genus X := finrank ℂ (HOF X)`
...
Ahlfors-Sario construction (see `Jacobians.Forms.Montel`). -/
...
`Module.finrank ℂ (HolomorphicOneForms X)` (see `Jacobians.Forms.Genus`), this
is `rfl`. -/
