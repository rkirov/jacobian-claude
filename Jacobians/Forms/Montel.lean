...
- `Jacobians/Forms/Cover.lean` — finite chart cover + compact shrinking.
- `Jacobians/Forms/LocalRep.lean` — chart-local scalar representative
  `localRep` + continuity + vector ops.
- `Jacobians/Forms/ChartNorm.lean` — per-chart bounded sup-norm
  `chartNormK` + triangle + homogeneity.
- `Jacobians/Forms/SupNorm.lean` — assembled sup-norm `supNormK` +
...
namespace Jacobians.Forms.Montel
...
`Jacobians/Forms/Genus.lean`). We wrap `supNormK` in an `AddGroupNorm` and use
...
higher level (as done in `Jacobians.Forms.HolomorphicForms`). -/
...
instances are in scope (typically provided in Jacobians.Forms.HolomorphicForms),
...
- All Arzelà-Ascoli infrastructure (B.1-B.9) is in
  `Jacobians/Forms/Compactness.lean`, fully proven.
