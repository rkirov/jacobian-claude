...
### NOTE: Removed false third conjunct (2026-05-28)

A prior version of this theorem contained a third conjunct asserting
that, for each `P`, the map `Q ↦ periodVec (sp P Q)` is `C^ω` into the
**unquotiented** space `Fin (genus X) → ℂ`. **This claim is
mathematically false** for any `X` with non-trivial `π_1` (i.e. any
positive-genus Riemann surface).

*Sketch of why it's false.* If a smooth (a fortiori continuous) `f : X →
ℂ^g` satisfied `f(Q) = periodVec γ_Q` for some path `γ_Q : P → Q` at
each `Q`, then by composing with the quotient map `π : ℂ^g → ℂ^g / Λ`,
`π ∘ f` would equal the Abel–Jacobi map `ofCurve P : X → Jacobian X`.
The existence of such a continuous lift `f` requires that the induced
homomorphism `(ofCurve P)_* : π_1(X) → π_1(ℂ^g / Λ) = Λ` be **zero**.
But this homomorphism is *exactly* the period map of homology classes,
which is non-zero by definition (the lattice `Λ` is generated by its
image). Contradiction.

The corresponding smoothness needed for `ofCurve_contMDiff` is the
**quotient** version (`Q ↦ [periodVec (sp P Q)]` smooth into the
Jacobian torus). That version is mathematically correct (the period
ambiguity is absorbed by the quotient) and is recorded as a separate
sorry, `ofCurve_contMDiff`, in `Jacobians.lean`. -/
