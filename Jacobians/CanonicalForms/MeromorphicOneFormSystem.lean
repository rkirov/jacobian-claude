/-
  ...
  `MeromorphicFunction` (`Jacobians.Meromorphic.Abel`) stores a bare map `toFun : X → ℂ` plus the predicate
  ...
  * `Ω_0 ⊇ HolomorphicOneForms` clean — a holomorphic form's `localRep` coefficient is **analytic**
  ...
  `linearSystem`/`lSysModule`/`lDim` pattern in `Jacobians.Meromorphic.LinearSystem`.
-/
