/-
  ...
  Built directly on `Jacobians.CanonicalForms.MeromorphicOneFormSystem`: the
  ...
  (the local-finiteness `orderAtPoint_isolated_at` of `Jacobians.Meromorphic.Abel`), and the construction of
  ...
  `linearSystem`/`lSysModule`/`lDim` pattern in `Jacobians.Meromorphic.LinearSystem`; the `Ω_D` build in
  `Jacobians.CanonicalForms.MeromorphicOneFormSystem`.
-/
import Jacobians.CanonicalForms.MeromorphicOneFormSystem
